 ...
 * @param int $contextid
 **/
function escape_import_question_files($field, $data, $answer, $contextid) {
    global $DB;
    if (!isset($data['itemid'])) {
        return;
    }
    $text = file_save_draft_area_files($data['itemid'],
            $contextid, 'mod_escape', 'page_' . $field . 's', $answer->id,
            array('subdirs' => false, 'maxfiles' => -1, 'maxbytes' => 0),
            $answer->$field);

    $DB->set_field("escape_answers", $field, $text, array("id" => $answer->id));
}
 ...
 **/
function escape_save_question_options($question, $escape, $contextid) {
    global $DB;
    ...
    switch ($question->qtype) {
        case ESCAPE_PAGE_SHORTANSWER:
            ...
            foreach ($question->answer as $key => $dataanswer) {
                if ($dataanswer != "") {
                    ...
                    escape_import_question_files('response', $question->feedback[$key], $answer, $contextid);
                    $answers[] = $answer->id;
            ...
            if ($maxfraction != 1) {
                ...
                $result->notice = get_string("fractionsnomax", "escape", $maxfraction);
                return $result;
            }
            break;

        case ESCAPE_PAGE_NUMERICAL:   // Note similarities to shortanswer.
        ...
        case ESCAPE_PAGE_TRUEFALSE:

            // In escape the correct answer always come first, as it was the case
            ...
            if ($question->correctanswer) {
                $answer->answer = get_string("true", "escape");
                if (isset($question->feedbacktrue)) {
                    ...
                    $answer->id = $DB->insert_record("escape_answers", $answer);
                    escape_import_question_files('response', $question->feedbacktrue, $answer, $contextid);
                }
            } else {
                $answer->answer = get_string("false", "escape");
                if (isset($question->feedbackfalse)) {
                    ...
                    $answer->id = $DB->insert_record("escape_answers", $answer);
                    escape_import_question_files('response', $question->feedbackfalse, $answer, $contextid);
                }
            }
        ...
        case ESCAPE_PAGE_MULTICHOICE:
            ...
            foreach ($question->answer as $key => $dataanswer) {
                if ($dataanswer != "") {
                    ...
                    $answer->responseformat = $question->feedback[$key]['format'];
                    $answer->id = $DB->insert_record("escape_answers", $answer);
                    escape_import_question_files('answer', $dataanswer, $answer, $contextid);
                    escape_import_question_files('response', $question->feedback[$key], $answer, $contextid);
            ...
            if ($question->single) {
                ...
                if ($totalfraction != 1) {
                    ...
                    $result->notice = get_string("fractionsaddwrong", "escape", $totalfraction);
                    return $result;
                }
            }
        break;

        case ESCAPE_PAGE_MATCHING:

            $subquestions = array();

            // The first answer should always be the correct answer
            $correctanswer = clone($defaultanswer);
            $correctanswer->answer = get_string('thatsthecorrectanswer', 'escape');
            $correctanswer->jumpto = ESCAPE_NEXTPAGE;
            $correctanswer->score = 1;
            $DB->insert_record("escape_answers", $correctanswer);

            // The second answer should always be the wrong answer
            $wronganswer = clone($defaultanswer);
            $wronganswer->answer = get_string('thatsthewronganswer', 'escape');
            $DB->insert_record("escape_answers", $wronganswer);
            ...
            foreach ($question->subquestions as $key => $questiontext) {
                ...
                if (!empty($questiontext) and !empty($answertext)) {
                    ...
                    $answer->id = $DB->insert_record("escape_answers", $answer);
                    escape_import_question_files('answer', $questiontext, $answer, $contextid);
            ...
            if (count($subquestions) < 3) {
                $result->notice = get_string("notenoughsubquestions", "escape");
                return $result;
            }
            break;

        case ESCAPE_PAGE_ESSAY:
            $answer = new stdClass();
            $answer->escapeid = $question->escapeid;
...
class qformat_default {
    ...
    var $qtypeconvert = array('numerical'   => ESCAPE_PAGE_NUMERICAL,
                               'multichoice' => ESCAPE_PAGE_MULTICHOICE,
                               'truefalse'   => ESCAPE_PAGE_TRUEFALSE,
                               'shortanswer' => ESCAPE_PAGE_SHORTANSWER,
                               'match'       => ESCAPE_PAGE_MATCHING,
                               'essay'       => ESCAPE_PAGE_ESSAY
                              );
    ...
    function importprocess($filename, $escape, $pageid) {
        global $DB, $OUTPUT;
    ...
    /// Processes a given file.  There's probably little need to change this
        ...
        echo $OUTPUT->notification(get_string('importcount', 'escape',
        ...
        if ($pageid == 0) {
            ...
            $updateescapepage = $DB->get_record('escape_pages', array('escapeid' => $escape->id, 'prevpageid' => 0));
        } else {
            $updateescapepage = $DB->get_record('escape_pages', array('escapeid' => $escape->id, 'id' => $pageid));
        }
                ...
                case 'essay' :
                    ...
                    if ($pageid) {
                        ...
                        if (!$page = $DB->get_record_select("escape_pages", "escapeid = :escapeid AND prevpageid = :prevpageid", $params)) {
                            ...
                            $newpageid = $DB->insert_record("escape_pages", $newpage);
                            // update the linked list
                            $DB->set_field("escape_pages", "prevpageid", $newpageid, array("id" => $page->id));
                        }
                    }
                    ...
                    if (isset($question->questiontextitemid)) {
                        $questiontext = file_save_draft_area_files($question->questiontextitemid,
                                $this->importcontext->id, 'mod_escape', 'page_contents', $newpageid,
                                null , $question->questiontext);
                        // Update content with recoded urls.
                        $DB->set_field("escape_pages", "contents", $questiontext, array("id" => $newpageid));
                    }

                    // Now to save all the answers and type-specific options

                    $question->escapeid = $escape->id; // needed for foreign key
                    $question->qtype = $this->qtypeconvert[$question->qtype];
                    $result = escape_save_question_options($question, $escape, $this->importcontext->id);
        ...
        if (!empty($updateescapepage)) {
            if ($addquestionontop) {
                $DB->set_field("escape_pages", "prevpageid", $pageid, array("id" => $updateescapepage->id));
            } else {
                $DB->set_field("escape_pages", "prevpageid", $pageid, array("id" => $updateescapepage->nextpageid));
            }
        }
        if ($unsupportedquestions) {
            echo $OUTPUT->notification(get_string('unknownqtypesnotimported', 'escape', $unsupportedquestions));
        }
        return true;
    }
    ...
    /**
     * Since the escape module tries to re-use the question bank import classes in
