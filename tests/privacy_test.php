...
use mod_escape\privacy\provider;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_escape_privacy_testcase extends provider_testcase {
    ...
    public function test_get_contexts_for_userid() {
        ...
        $cm1 = $dg->create_module('escape', ['course' => $c1]);
        $cm2 = $dg->create_module('escape', ['course' => $c1]);
        $cm3 = $dg->create_module('escape', ['course' => $c1]);
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        $assertcm1nochange = function() use ($DB, $u1, $u2, $cm1) {
            $this->assertTrue($DB->record_exists('escape_attempts', ['userid' => $u1->id, 'escapeid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('escape_grades', ['userid' => $u1->id, 'escapeid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('escape_timer', ['userid' => $u1->id, 'escapeid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('escape_branch', ['userid' => $u1->id, 'escapeid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('escape_overrides', ['userid' => $u1->id, 'escapeid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('escape_attempts', ['userid' => $u2->id, 'escapeid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('escape_grades', ['userid' => $u2->id, 'escapeid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('escape_timer', ['userid' => $u2->id, 'escapeid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('escape_branch', ['userid' => $u2->id, 'escapeid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('escape_overrides', ['userid' => $u2->id, 'escapeid' => $cm1->id]));
        };
        $assertcm2nochange = function() use ($DB, $u1, $u2, $cm2) {
            $this->assertTrue($DB->record_exists('escape_attempts', ['userid' => $u1->id, 'escapeid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('escape_grades', ['userid' => $u1->id, 'escapeid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('escape_timer', ['userid' => $u1->id, 'escapeid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('escape_branch', ['userid' => $u1->id, 'escapeid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('escape_overrides', ['userid' => $u1->id, 'escapeid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('escape_attempts', ['userid' => $u2->id, 'escapeid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('escape_grades', ['userid' => $u2->id, 'escapeid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('escape_timer', ['userid' => $u2->id, 'escapeid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('escape_branch', ['userid' => $u2->id, 'escapeid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('escape_overrides', ['userid' => $u2->id, 'escapeid' => $cm2->id]));
        };
        ...
        $assertcm2nochange();
        $this->assertFalse($DB->record_exists('escape_attempts', ['userid' => $u1->id, 'escapeid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('escape_grades', ['userid' => $u1->id, 'escapeid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('escape_timer', ['userid' => $u1->id, 'escapeid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('escape_branch', ['userid' => $u1->id, 'escapeid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('escape_overrides', ['userid' => $u1->id, 'escapeid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('escape_attempts', ['userid' => $u2->id, 'escapeid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('escape_grades', ['userid' => $u2->id, 'escapeid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('escape_timer', ['userid' => $u2->id, 'escapeid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('escape_branch', ['userid' => $u2->id, 'escapeid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('escape_overrides', ['userid' => $u2->id, 'escapeid' => $cm1->id]));
    }
    ...
    public function test_delete_data_for_user() {
        ...
        // Delete the course: no change.
        provider::delete_data_for_user(new approved_contextlist($u1, 'mod_escape', [context_course::instance($c1->id)->id]));
        $assertu1nochange();
        $assertu2nochange();

        // Delete another module: no change.
        provider::delete_data_for_user(new approved_contextlist($u1, 'mod_escape', [context_module::instance($cm3->cmid)->id]));
        $assertu1nochange();
        $assertu2nochange();

        // Delete u1 in cm1: no change for u2 and in cm2.
        provider::delete_data_for_user(new approved_contextlist($u1, 'mod_escape', [context_module::instance($cm1->cmid)->id]));
        $assertu2nochange();
        $this->assertFalse($DB->record_exists('escape_attempts', ['userid' => $u1->id, 'escapeid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('escape_grades', ['userid' => $u1->id, 'escapeid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('escape_timer', ['userid' => $u1->id, 'escapeid' => $cm1->id]));
    ...
    public function test_delete_data_for_users() {
        ...
        $assertnochange = function($user, $cm) use ($DB) {
            $this->assertTrue($DB->record_exists('escape_attempts', ['userid' => $user->id, 'escapeid' => $cm->id]));
            $this->assertTrue($DB->record_exists('escape_grades', ['userid' => $user->id, 'escapeid' => $cm->id]));
            $this->assertTrue($DB->record_exists('escape_timer', ['userid' => $user->id, 'escapeid' => $cm->id]));
            $this->assertTrue($DB->record_exists('escape_branch', ['userid' => $user->id, 'escapeid' => $cm->id]));
            $this->assertTrue($DB->record_exists('escape_overrides', ['userid' => $user->id, 'escapeid' => $cm->id]));
        };

        $assertdeleted = function($user, $cm) use ($DB) {
            $this->assertFalse($DB->record_exists('escape_attempts', ['userid' => $user->id, 'escapeid' => $cm->id]));
            $this->assertFalse($DB->record_exists('escape_grades', ['userid' => $user->id, 'escapeid' => $cm->id]));
            $this->assertFalse($DB->record_exists('escape_timer', ['userid' => $user->id, 'escapeid' => $cm->id]));
            $this->assertFalse($DB->record_exists('escape_branch', ['userid' => $user->id, 'escapeid' => $cm->id]));
            $this->assertFalse($DB->record_exists('escape_overrides', ['userid' => $user->id, 'escapeid' => $cm->id]));
        };
        ...
        // Delete another module: no change.
        $approveduserlist = new core_privacy\local\request\approved_userlist($context3, 'mod_escape', [$u1->id]);
        ...
        $approveduserlist = new core_privacy\local\request\approved_userlist($context1, 'mod_escape', [$u1->id]);
        provider::delete_data_for_users($approveduserlist);
    ...
    public function test_export_data_for_user_overrides() {
        ...
        provider::export_user_data(new approved_contextlist($u1, 'mod_escape', [$cm1ctx->id, $cm2ctx->id]));
        ...
        writer::reset();
        provider::export_user_data(new approved_contextlist($u2, 'mod_escape', [$cm1ctx->id, $cm2ctx->id]));
        $data = writer::with_context($cm1ctx)->get_data([]);
        $this->assertNotEmpty($data);
        $data = writer::with_context($cm1ctx)->get_related_data([], 'overrides');
    ...
    public function test_export_data_for_user_timers() {
        ...
        $now = time();
        $this->create_timer($cm2, $u1, ['starttime' => $now - 2000, 'escapetime' => $now + 3600, 'completed' => 0,
            'timemodifiedoffline' => $now - 7000]);
        $this->create_timer($cm2, $u1, ['starttime' => $now - 1000, 'escapetime' => $now + 1600, 'completed' => 0]);
        $this->create_timer($cm2, $u1, ['starttime' => $now - 500, 'escapetime' => $now + 100, 'completed' => 1]);
        $this->create_timer($cm1, $u2, ['starttime' => $now - 1000, 'escapetime' => $now + 1800, 'completed' => 1]);
    ...
    public function test_export_data_for_user_attempts() {
        global $DB;
        $dg = $this->getDataGenerator();
        $lg = $dg->get_plugin_generator('mod_escape');
        ...
        $page4 = $lg->create_question_multichoice($cm1, [
            ...
            'jumpto' => [ESCAPE_NEXTPAGE, ESCAPE_NEXTPAGE, ESCAPE_THISPAGE]
        ]);
        $page4answers = array_keys($DB->get_records('escape_answers', ['pageid' => $page4->id], 'id'));
        ...
        $this->assertEquals(get_string('endofescape', 'mod_escape'), $jump['went_to']);
     ...
     *
     * @param object $escape The escape.
     * @param object $user The user.
     * @param array $options Options.
     * @return object
     */
    protected function create_attempt($escape, $user, array $options = []) {
        global $DB;
        ...
        $record->id = $DB->insert_record('escape_attempts', $record);
        return $record;
    }
     ...
     * @return object
     */
    protected function create_grade($escape, $user, array $options = []) {
        global $DB;
        $record = (object) array_merge([
            'escapeid' => $escape->id,
            'userid' => $user->id,
        ...
        $record->id = $DB->insert_record('escape_grades', $record);
        return $record;
    }
     ...
     * @return object
     */
    protected function create_timer($escape, $user, array $options = []) {
        global $DB;
        $record = (object) array_merge([
            'escapeid' => $escape->id,
            'userid' => $user->id,
        ...
        $record->id = $DB->insert_record('escape_timer', $record);
        return $record;
    }
     ...
     * @return object
     */
    protected function create_branch($escape, $user, array $options = []) {
        global $DB;
        $record = (object) array_merge([
            'escapeid' => $escape->id,
            'userid' => $user->id,
        ...
        $record->id = $DB->insert_record('escape_branch', $record);
        return $record;
    }
     ...
     * @return object
     */
    protected function create_override($escape, $user, array $options = []) {
        global $DB;
        $record = (object) array_merge([
            'escapeid' => $escape->id,
            'userid' => $user->id,
        ], $options);
        $record->id = $DB->insert_record('escape_overrides', $record);
        return $record;
    }
}
