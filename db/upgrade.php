 ...
 * @return bool
 */
function xmldb_escape_upgrade($oldversion) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2016120515) {
        // Define new fields to be added to escape.
        $table = new xmldb_table('escape');
        ...
        // Escape savepoint reached.
        upgrade_mod_savepoint(true, 2016120515, 'escape');
    }
    if ($oldversion < 2016120516) {
        // New field for escape_timer.
        $table = new xmldb_table('escape_timer');
        ...
        // Escape savepoint reached.
        upgrade_mod_savepoint(true, 2016120516, 'escape');
    }
    ...
    if ($oldversion < 2017051501) {
        ...
        // Delete orphaned escape answer and response files.
        $sql = "SELECT DISTINCT f.contextid, f.component, f.filearea, f.itemid
                  FROM {files} f
             LEFT JOIN {escape_answers} la ON f.itemid = la.id
                 WHERE component = :component
                   AND (filearea = :fileareaanswer OR filearea = :filearearesponse)
                   AND la.id IS NULL";

        $orphanedfiles = $DB->get_recordset_sql($sql, array('component' => 'mod_escape', 'fileareaanswer' => 'page_answers',
        ...
        upgrade_mod_savepoint(true, 2017051501, 'escape');
    }
