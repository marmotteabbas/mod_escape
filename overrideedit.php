...
/**
 * This page handles editing and creation of escape overrides
 *
 * @package   mod_escape
...
if ($overrideid) {
    ...
    if (! $override = $DB->get_record('escape_overrides', array('id' => $overrideid))) {
        print_error('invalidoverrideid', 'escape');
    }

    $escape = new escape($DB->get_record('escape', array('id' => $override->escapeid), '*',  MUST_EXIST));

    list($course, $cm) = get_course_and_cm_from_instance($escape, 'escape');
...
$url = new moodle_url('/mod/escape/overrideedit.php');
if ($action) {
...
// Merge escape defaults with data.
...
$overridelisturl = new moodle_url('/mod/escape/overrides.php', array('cmid' => $cm->id));
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new escape_override_form($url, $cm, $escape, $context, $groupmode, $override);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    // Process the data.
    $fromform->escapeid = $escape->id;

    // Replace unchanged values with null.
    foreach ($keys as $key) {
        if ($fromform->{$key} == $escape->{$key}) {
            $fromform->{$key} = null;
        }
    }
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('escape_overrides', $conditions)) {
            ...
            $escape->delete_override($oldoverride->id);
        }
    }
    ...
    if (!empty($override->id)) {
        ...
        $DB->update_record('escape_overrides', $fromform);
        ...
        unset($fromform->id);
        $fromform->id = $DB->insert_record('escape_overrides', $fromform);
    ...
    if ($groupmode) {
        ...
        escape_update_events($escape, $fromform);
    }
...
// Print the form.
$pagetitle = get_string('editoverride', 'escape');
