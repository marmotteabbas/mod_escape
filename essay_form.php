...
class essay_grading_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'formheader', get_string('question', 'escape'));
        ...
        $mform->addElement('static', 'question', get_string('question', 'escape'));
        $mform->addElement('static', 'studentanswer', get_string('studentresponse', 'escape', fullname($this->_customdata['user'], true)));

        $editoroptions = array('noclean' => true, 'maxfiles' => EDITOR_UNLIMITED_FILES, 'maxbytes' => $CFG->maxbytes);
        $mform->addElement('editor', 'response_editor', get_string('comments', 'escape'), null, $editoroptions);
        $mform->setType('response', PARAM_RAW);

        $mform->addElement('select', 'score', get_string('essayscore', 'escape'), $this->_customdata['scoreoptions']);
