...
/**
 * Local library file for Escape.  These are non-standard functions that are used
 * only by Escape.
 *
 * @package mod_escape
...
/** This page */
define('ESCAPE_THISPAGE', 0);
/** Next page -> any page not seen before */
define("ESCAPE_UNSEENPAGE", 1);
/** Next page -> any page not answered correctly */
define("ESCAPE_UNANSWEREDPAGE", 2);
/** Jump to Next Page */
define("ESCAPE_NEXTPAGE", -1);
/** End of Escape */
define("ESCAPE_EOL", -9);
/** Jump to an unseen page within a branch and end of branch or end of escape */
define("ESCAPE_UNSEENBRANCHPAGE", -50);
/** Jump to Previous Page */
define("ESCAPE_PREVIOUSPAGE", -40);
/** Jump to a random page within a branch and end of branch or end of escape */
define("ESCAPE_RANDOMPAGE", -60);
/** Jump to a random Branch */
define("ESCAPE_RANDOMBRANCH", -70);
/** Cluster Jump */
define("ESCAPE_CLUSTERJUMP", -80);
/** Undefined */
define("ESCAPE_UNDEFINED", -99);

/** ESCAPE_MAX_EVENT_LENGTH = 432000 ; 5 days maximum */
define("ESCAPE_MAX_EVENT_LENGTH", "432000");

/** Answer format is HTML */
define("ESCAPE_ANSWER_HTML", "HTML");

//////////////////////////////////////////////////////////////////////////////////////
/// Any other escape functions go here.  Each of them must have a name that
/// starts with escape_

/**
 * Checks to see if a ESCAPE_CLUSTERJUMP or
 * a ESCAPE_UNSEENBRANCHPAGE is used in a escape.
 *
 * This function is only executed when a teacher is
 * checking the navigation for a escape.
 *
 * @param stdClass $escape Id of the escape that is to be checked.
 * @return boolean True or false.
 **/
function escape_display_teacher_warning($escape) {
    global $DB;

    // get all of the escape answers
    $params = array ("escapeid" => $escape->id);
    if (!$escapeanswers = $DB->get_records_select("escape_answers", "escapeid = :escapeid", $params)) {
        // no answers, then not using cluster or unseen
        return false;
    }
    // just check for the first one that fulfills the requirements
    foreach ($escapeanswers as $escapeanswer) {
        if ($escapeanswer->jumpto == ESCAPE_CLUSTERJUMP || $escapeanswer->jumpto == ESCAPE_UNSEENBRANCHPAGE) {
            return true;
        }
    }
...
/**
 * Interprets the ESCAPE_UNSEENBRANCHPAGE jump.
 *
 ...
 **/
function escape_unseen_question_jump($escape, $user, $pageid) {
    global $DB;

    // get the number of retakes
    if (!$retakes = $DB->count_records("escape_grades", array("escapeid"=>$escape->id, "userid"=>$user))) {
        $retakes = 0;
    }

    // get all the escape_attempts aka what the user has seen
    if ($viewedpages = $DB->get_records("escape_attempts", array("escapeid"=>$escape->id, "userid"=>$user, "retry"=>$retakes), "timeseen DESC")) {
    ...
    // get the escape pages
    $escapepages = $escape->load_all_pages();

    if ($pageid == ESCAPE_UNSEENBRANCHPAGE) {  // this only happens when a student leaves in the middle of an unseen question within a branch series
    ...
    while ($pageid != 0) { // this condition should never be satisfied... only happens if there are no branch tables above this page
        if ($escapepages[$pageid]->qtype == ESCAPE_PAGE_BRANCHTABLE) {
            break;
        }
        $pageid = $escapepages[$pageid]->prevpageid;
    }

    $pagesinbranch = $escape->get_sub_pages_of($pageid, array(ESCAPE_PAGE_BRANCHTABLE, ESCAPE_PAGE_ENDOFBRANCH));
    ...
    if(count($unseen) == 0) {
        if(isset($pagesinbranch)) {
            ...
            $nextpage = $escapepages[$pageid]->nextpageid;
        }
        if ($nextpage == 0) {
            return ESCAPE_EOL;
        } else {
 ...
 * @return int Will return the page id of a branch table or end of escape
 **/
function escape_unseen_branch_jump($escape, $userid) {
    global $DB;

    if (!$retakes = $DB->count_records("escape_grades", array("escapeid"=>$escape->id, "userid"=>$userid))) {
        $retakes = 0;
    }

    if (!$seenbranches = $escape->get_content_pages_viewed($retakes, $userid, 'timeseen DESC')) {
        print_error('cannotfindrecords', 'escape');
    }
    ...
    // this function searches through the escape pages to find all the branch tables
    // that follow the flagged branch table
    $pageid = $escapepages[$start]->nextpageid; // move down from the flagged branch table
    $branchtables = array();
    while ($pageid != 0) {  // grab all of the branch table till eol
        if ($escapepages[$pageid]->qtype == ESCAPE_PAGE_BRANCHTABLE) {
            $branchtables[] = $escapepages[$pageid]->id;
        }
        $pageid = $escapepages[$pageid]->nextpageid;
    }
    $unseen = array();
    ...
    if (count($unseen) > 0) {
        ...
        return ESCAPE_EOL;  // has viewed all of the branch tables
    }
}

/**
 * Handles the random jump between a branch table and end of branch or end of escape (ESCAPE_RANDOMPAGE).
 *
 * @param escape $escape
 ...
 **/
function escape_random_question_jump($escape, $pageid) {
    global $DB;

    // get the escape pages
    $params = array ("escapeid" => $escape->id);
    if (!$escapepages = $DB->get_records_select("escape_pages", "escapeid = :escapeid", $params)) {
    ...
    if(count($pagesinbranch) == 0) {
        // there are no pages inside the branch, so return the next page
        return $escapepages[$pageid]->nextpageid;
    } else {
...
/**
 * Calculates a user's grade for a escape.
 *
 ...
 */
function escape_grade($escape, $ntries, $userid = 0) {
    global $USER, $DB;
    ...
    $params = array ("escapeid" => $escape->id, "userid" => $userid, "retry" => $ntries);
    if ($useranswers = $DB->get_records_select("escape_attempts",  "escapeid = :escapeid AND
        ...
        // Drop all attempts that go beyond max attempts for the escape
        foreach ($attemptset as $key => $set) {
            $attemptset[$key] = array_slice($set, 0, $escape->maxattempts);
        }
        ...
        $pages = $DB->get_records_select("escape_pages", "escapeid = ? AND id $usql", $parameters);
        $answers = $DB->get_records_select("escape_answers", "escapeid = ? AND pageid $usql", $parameters);

        // Number of pages answered
        $nquestions = count($pages);

        foreach ($attemptset as $attempts) {
            $page = escape_page::load($pages[end($attempts)->pageid], $escape);
            if ($escape->custom) {
        ...
        if ($escape->custom) {
            ...
            if ($escape->minquestions and $nquestions < $escape->minquestions) {
                // Nope, increase number viewed by the amount of unanswered questions
                $total =  $nviewed + ($escape->minquestions - $nquestions);
            } else {
                $total = $nviewed;
            }
        }
 ...
 * is whether a user has a grade greater than or equal to the escape setting
 * of displayleftif
 *
 * @param object $escape Escape object of the current escape
 * @return boolean 0 if the user cannot see, or $escape->displayleft to keep displayleft unchanged
 **/
function escape_displayleftif($escape) {
    global $CFG, $USER, $DB;

    if (!empty($escape->displayleftif)) {
        // get the current user's max grade for this escape
        $params = array ("userid" => $USER->id, "escapeid" => $escape->id);
        if ($maxgrade = $DB->get_record_sql('SELECT userid, MAX(grade) AS maxgrade FROM {escape_grades} WHERE userid = :userid AND escapeid = :escapeid GROUP BY userid', $params)) {
            if ($maxgrade->maxgrade < $escape->displayleftif) {
    ...
    // if we get to here, keep the original state of displayleft escape setting
    return $escape->displayleft;
}
 ...
 * @return unknown_type
 */
function escape_add_fake_blocks($page, $cm, $escape, $timer = null) {
    $bc = escape_menu_block_contents($cm->id, $escape);
    if (!empty($bc)) {
    ...
    $bc = escape_mediafile_block_contents($cm->id, $escape);
    if (!empty($bc)) {
        $page->blocks->add_fake_block($bc, $page->blocks->get_default_region());
    }

    if (!empty($timer)) {
        $bc = escape_clock_block_contents($cm->id, $escape, $timer, $page);
        if (!empty($bc)) {
            $page->blocks->add_fake_block($bc, $page->blocks->get_default_region());
        }
    }
 ...
 * escape, return a block_contents that displays it.
 *
 * @param int $cmid Course Module ID for this escape
 * @param object $escape Full escape record object
 * @return block_contents
 **/
function escape_mediafile_block_contents($cmid, $escape) {
    global $OUTPUT;
    if (empty($escape->mediafile)) {
        return null;
    }
    ...
    $options['resizable'] = 1;
    $options['width'] = $escape->mediawidth;
    $options['height'] = $escape->mediaheight;

    $link = new moodle_url('/mod/escape/mediafile.php?id='.$cmid);
    $action = new popup_action('click', $link, 'escapemediafile', $options);
    $content = $OUTPUT->action_link($link, get_string('mediafilepopup', 'escape'), $action, array('title'=>get_string('mediafilepopup', 'escape')));

    $bc = new block_contents();
    $bc->title = get_string('linkedmedia', 'escape');
...
/**
 * If a timed escape and not a teacher, then
 ...
 * @return block_contents
 **/
function escape_clock_block_contents($cmid, $escape, $timer, $page) {
    // Display for timed escapes and for students only
    $context = context_module::instance($cmid);
    if ($escape->timelimit == 0 || has_capability('mod/escape:manage', $context)) {
        return null;
    }

    $content = '<div id="escape-timer">';
    $content .=  $escape->time_remaining($timer->starttime);
    $content .= '</div>';

    $clocksettings = array('starttime' => $timer->starttime, 'servertime' => time(), 'testlength' => $escape->timelimit);
    $page->requires->data_for_js('clocksettings', $clocksettings, true);
    $page->requires->strings_for_js(array('timeisup'), 'escape');
    $page->requires->js('/mod/escape/timer.js');
    $page->requires->js_init_call('show_clock');

    $bc = new block_contents();
    $bc->title = get_string('timeremaining', 'escape');
 ...
 *
 * @param int $cmid Course Module ID for this escape
 * @param escape $escape Full escape record object
 * @return void
 **/
function escape_menu_block_contents($cmid, $escape) {
    global $CFG, $DB;
    ...
    while ($pageid != 0) {
        ...
        if ($page->displayinmenublock && $page->display) {
            if ($page->id == $currentpageid) {
                ...
                $content .= "<li class=\"notselected\"><a href=\"$CFG->wwwroot/mod/escape/view.php?id=$cmid&amp;pageid=$page->id\">".format_string($page->title,true)."</a></li>\n";
            }
    ...
    $bc = new block_contents();
    $bc->title = get_string('escapemenu', 'escape');
...
/**
 * Adds header buttons to the page for the escape
 *
 * @param object $cm
 * @param object $context
 * @param bool $extraeditbuttons
 * @param int $escapepageid
 */
function escape_add_header_buttons($cm, $context, $extraeditbuttons=false, $escapepageid=null) {
    global $CFG, $PAGE, $OUTPUT;
    if (has_capability('mod/escape:edit', $context) && $extraeditbuttons) {
        if ($escapepageid === null) {
            print_error('invalidpageid', 'escape');
        }
        if (!empty($escapepageid) && $escapepageid != ESCAPE_EOL) {
            $url = new moodle_url('/mod/escape/editpage.php', array(
                'id'       => $cm->id,
                'pageid'   => $escapepageid,
                'edit'     => 1,
                'returnto' => $PAGE->url->out(false)
            ));
            $PAGE->set_button($OUTPUT->single_button($url, get_string('editpagecontent', 'escape')));
        }
    }
}
 ...
 */
function escape_get_media_html($escape, $context) {
    global $CFG, $PAGE, $OUTPUT;
    require_once("$CFG->libdir/resourcelib.php");

    // get the media file link
    if (strpos($escape->mediafile, '://') !== false) {
        $url = new moodle_url($escape->mediafile);
    } else {
        // the timemodified is used to prevent caching problems, instead of '/' we should better read from files table and use sortorder
        $url = moodle_url::make_pluginfile_url($context->id, 'mod_escape', 'mediafile', $escape->timemodified, '/', ltrim($escape->mediafile, '/'));
    }
    $title = $escape->mediafile;
 ...
 * @return void
 */
function escape_process_group_deleted_in_course($courseid, $groupid = null) {
    global $DB;
    ...
    if ($groupid) {
        ...
        $sql = "SELECT o.id, o.escapeid
                  FROM {escape_overrides} o
                  JOIN {escape} escape ON escape.id = o.escapeid
                 WHERE escape.course = :courseid
                   AND o.groupid = :groupid";
    } else {
        // No groupid, we update all orphaned group overrides for all escapes in course.
        $sql = "SELECT o.id, o.escapeid
                  FROM {escape_overrides} o
                  JOIN {escape} escape ON escape.id = o.escapeid
    ...
    $DB->delete_records_list('escape_overrides', 'id', array_keys($records));
}
 ...
 * @since  Moodle 3.3
 */
function escape_get_overview_report_table_and_data(escape $escape, $currentgroup) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/mod/escape/pagetypes/branchtable.php');

    $context = $escape->context;
    $cm = $escape->cm;
    // Count the number of branch and question pages in this escape.
    $branchcount = $DB->count_records('escape_pages', array('escapeid' => $escape->id, 'qtype' => ESCAPE_PAGE_BRANCHTABLE));
    $questioncount = ($DB->count_records('escape_pages', array('escapeid' => $escape->id)) - $branchcount);

    // Only load students if there attempts for this escape.
    $attempts = $DB->record_exists('escape_attempts', array('escapeid' => $escape->id));
    $branches = $DB->record_exists('escape_branch', array('escapeid' => $escape->id));
    $timer = $DB->record_exists('escape_timer', array('escapeid' => $escape->id));
    if ($attempts or $branches or $timer) {
        ...
        $params['a1escapeid'] = $escape->id;
        $params['b1escapeid'] = $escape->id;
        $params['c1escapeid'] = $escape->id;
        $ufields = user_picture::fields('u', $extrafields);
        $sql = "SELECT DISTINCT $ufields
                FROM {user} u
                JOIN (
                    SELECT userid, escapeid FROM {escape_attempts} a1
                    WHERE a1.escapeid = :a1escapeid
                        UNION
                    SELECT userid, escapeid FROM {escape_branch} b1
                    WHERE b1.escapeid = :b1escapeid
                        UNION
                    SELECT userid, escapeid FROM {escape_timer} c1
                    WHERE c1.escapeid = :c1escapeid
                    ) a ON u.id = a.userid
    ...
    if (! $grades = $DB->get_records('escape_grades', array('escapeid' => $escape->id), 'completed')) {
        $grades = array();
    }

    if (! $times = $DB->get_records('escape_timer', array('escapeid' => $escape->id), 'starttime')) {
        $times = array();
    }
    ...
    $attempts = $DB->get_recordset('escape_attempts', array('escapeid' => $escape->id), 'timeseen');
    foreach ($attempts as $attempt) {
        ...
        if (empty($studentdata[$attempt->userid]) || empty($studentdata[$attempt->userid][$attempt->retry])) {
            ...
            // build up the array.
            // this array represents each student and all of their tries at the escape
    ...
    $branches = $DB->get_recordset('escape_branch', array('escapeid' => $escape->id), 'timeseen');
    ...
    // Determine if escape should have a score.
    if ($branchcount > 0 AND $questioncount == 0) {
        // This escape only contains content pages and is not graded.
        $data->escapescored = false;
    } else {
        // This escape is graded.
        $data->escapescored = true;
    }
    ...
    $headers [] = get_string('attempts', 'escape');

    // Set up the table object.
    if ($data->escapescored) {
        $headers [] = get_string('highscore', 'escape');
    }
    ...
    // going through each student that has attempted the escape, so, each student should have something to be displayed
        ...
        if (array_key_exists($student->id, $studentdata)) {
            ...
            foreach ($tries as $try) {
                ...
                $temp .= "<a href=\"report.php?id=$cm->id&amp;action=reportdetail&amp;userid=".$try['userid']
                        .'&amp;try='.$try['try'].'" class="escape-attempt-link">';
                if ($try["grade"] !== null) { // if null then not done yet
                    ...
                    if ($try["end"]) {
                        // User finished the escape but has no grade. (Happens when there are only content pages).
                        $temp .= "&nbsp;".userdate($try["timestart"]);
                        $timetotake = $try["timeend"] - $try["timestart"];
                        $temp .= ",&nbsp;(".format_time($timetotake).")</a>";
                    } else {
                        // This is what the link does/looks like when the user has not completed the attempt.
                        $temp .= get_string("notcompleted", "escape");
                        if ($try['timestart'] !== 0) {
                ...
                // Run these lines for the stats only if the user finnished the escape.
                if ($try["end"]) {
                    // User has completed the escape.
                    $data->numofattempts++;
            ...
            if ($data->escapescored) {
                // Add the grade if the escape is graded.
                $row[] = $bestgrade."%";
            }
 ...
 * @since  Moodle 3.3
 */
function escape_get_user_detailed_report_data(escape $escape, $userid, $attempt) {
    global $DB;
    ...
    $pageid = 0;
    $escapepages = $escape->load_all_pages();
    foreach ($escapepages as $escapepage) {
        if ($escapepage->prevpageid == 0) {
            $pageid = $escapepage->id;
        }
    }
    ...
    $pagestats = array();
    while ($pageid != 0) { // EOL
        $page = $escapepages[$pageid];
        $params = array ("escapeid" => $escape->id, "pageid" => $page->id);
        if ($allanswers = $DB->get_records_select("escape_attempts", "escapeid = :escapeid AND pageid = :pageid", $params, "timeseen")) {
    ...
    while ($pageid != 0) { // EOL
        ...
        if (empty($userid)) {
            ...
            $useranswer = null;
        } elseif ($useranswers = $DB->get_records("escape_attempts",array("escapeid"=>$escape->id, "userid"=>$userid, "retry"=>$attempt,"pageid"=>$page->id), "timeseen")) {
    ...
    if (!empty($userid)) {
        $params = array("escapeid"=>$escape->id, "userid"=>$userid);

        $alreadycompleted = true;

        if (!$grades = $DB->get_records_select("escape_grades", "escapeid = :escapeid and userid = :userid", $params, "completed", "*", $attempt, 1)) {
            $userstats->grade = -1;
        ...
        if (!$times = $escape->get_user_timers($userid, 'starttime', '*', $attempt, 1)) {
            $userstats->timetotake = -1;
            $alreadycompleted = false;
        } else {
            $userstats->timetotake = current($times);
            $userstats->timetotake = $userstats->timetotake->escapetime - $userstats->timetotake->starttime;
        }

        if ($alreadycompleted) {
            $userstats->gradeinfo = escape_grade($escape, $attempt, $userid);
        }
    }
...
/**
 * Return user's deadline for all escapes in a course, hereby taking into account group and user overrides.
 *
 * @param int $courseid the course id.
 * @return object An object with of all escapesids and close unixdates in this course,
 * taking into account the most lenient overrides, if existing and 0 if no close date is set.
 */
function escape_get_user_deadline($courseid) {
    global $DB, $USER;

    // For teacher and manager/admins return escape's deadline.
    if (has_capability('moodle/course:update', context_course::instance($courseid))) {
        $sql = "SELECT escape.id, escape.deadline AS userdeadline
                  FROM {escape} escape
                 WHERE escape.course = :courseid";
    ...
    $sql = "SELECT a.id,
              ...
              FROM (
                      SELECT escape.id as escapeid,
                             MAX(leo.deadline) AS userclose, MAX(qgo.deadline) AS groupclose
                        FROM {escape} escape
                   LEFT JOIN {escape_overrides} leo on escape.id = leo.escapeid AND leo.userid = :userid
                   LEFT JOIN {groups_members} gm ON gm.userid = :useringroupid
                   LEFT JOIN {escape_overrides} qgo on escape.id = qgo.escapeid AND qgo.groupid = gm.groupid
                       WHERE escape.course = :courseid
                    GROUP BY escape.id
                   ) v
              JOIN {escape} a ON a.id = v.escapeid";
 ...
 */
abstract class escape_add_page_form_base extends moodleform {

    /**
     * This is the classic define that is used to identify this pagetype.
     * Will be one of ESCAPE_*
     * @var int
     */
    public $qtype;
    ...
    public final function definition() {
        ...
        $PAGE->requires->css('/mod/escape/style/leaflet.css');
        $PAGE->requires->js_call_amd('mod_escape/map_pilot', 'init',array());
        ...
        if ($this->qtypestring != 'selectaqtype') {
            if ($this->_customdata['edit']) {
                $mform->addElement('header', 'qtypeheading', get_string('edit'. $this->qtypestring, 'escape'));
            } else {
                $mform->addElement('header', 'qtypeheading', get_string('add'. $this->qtypestring, 'escape'));
            }
        }
        ...
        if ($this->_customdata['edit'] === true) {
            ...
            $this->add_action_buttons(get_string('cancel'), get_string('savepage', 'escape'));
        } else if ($this->qtype === 'questiontype') {
            $this->add_action_buttons(get_string('cancel'), get_string('addaquestionpage', 'escape'));
        } else {
            $this->add_action_buttons(get_string('cancel'), get_string('savepage', 'escape'));
        }
    }
     ...
     */
    protected final function add_jumpto($name, $label=null, $selected=ESCAPE_NEXTPAGE) {
        $title = get_string("jump", "escape");
        if ($label === null) {
            $label = $title;
        }
        if (is_int($name)) {
        ...
        $this->_form->addHelpButton($name, 'jumps', 'escape');
    }
    ...
    protected final function add_answer($count, $label = null, $required = false, $format= '') {
        if ($label === null) {
            $label = get_string('answer', 'escape');
        }

        if ($format == ESCAPE_ANSWER_HTML) {
    ...
    protected final function add_response($count, $label = null, $required = false) {
        if ($label === null) {
            $label = get_string('response', 'escape');
        }
...
/**
 * Class representation of a escape
 *
 * This class is used the interact with, and manage a escape once instantiated.
 * If you need to fetch a escape object you can do so by calling
 *
 * <code>
 * escape::load($escapeid);
 * // or
 * $escaperecord = $DB->get_record('escape', $escapeid);
 * $escape = new escape($escaperecord);
 * </code>
 *
 * The class itself extends escape_base as all classes within the escape module should
 *
 * These properties are from the database
 * @property int $id The id of this escape
 * @property int $course The ID of the course this escape belongs to
 * @property string $name The name of this escape
 * @property int $practice Flag to toggle this as a practice escape
 ...
 * @property int $dependency ID of another escape this escape is dependent on
 * @property string $conditions Conditions of the escape dependency
 ...
 * @property int $maxpages Maximum number of pages this escape can contain
 * @property int $retake Flag to allow users to retake a escape
 * @property int $activitylink Relate this escape to another escape
 ...
 * @property int $progressbar Flag to toggle display of a escape progress bar
 * @property int $available Timestamp of when this escape becomes available
 * @property int $deadline Timestamp of when this escape is no longer available
 * @property int $timemodified Timestamp when escape was last modified
 * @property int $allowofflineattempts Whether to allow the escape to be attempted offline in the mobile app
 *
 * These properties are calculated
 * @property int $firstpageid Id of the first page of this escape (prevpageid=0)
 * @property int $lastpageid Id of the last page of this escape (nextpageid=0)
 *
 * @copyright  2009 Sam Hemelryk
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class escape extends escape_base {

    /**
     * The id of the first page (where prevpageid = 0) gets set and retrieved by
     * {@see get_firstpageid()} by directly calling <code>$escape->firstpageid;</code>
     * @var int
     */
    protected $firstpageid = null;
    /**
     * The id of the last page (where nextpageid = 0) gets set and retrieved by
     * {@see get_lastpageid()} by directly calling <code>$escape->lastpageid;</code>
     * @var int
     */
    protected $lastpageid = null;
    /**
     * An array used to cache the pages associated with this escape after the first
     * time they have been loaded.
     * A note to developers: If you are going to be working with MORE than one or
     * two pages from a escape you should probably call {@see $escape->load_all_pages()}
     * in order to save excess database queries.
     * @var array An array of escape_page objects
     */
    protected $pages = array();
    /**
     * Flag that gets set to true once all of the pages associated with the escape
     * have been loaded.
     * @var bool
     */
    ...
    /**
     * Course module object gets set and retrieved by directly calling <code>$escape->cm;</code>
     * @see get_cm()
     * @var stdClass
     */
    protected $cm = null;
    ...
    /**
     * Course object gets set and retrieved by directly calling <code>$escape->courserecord;</code>
    ...
    /**
     * Context object gets set and retrieved by directly calling <code>$escape->context;</code>
     * @see get_context()
     * @var stdClass
     */
    ...
    /**
     * Simply generates a escape object given an array/object of properties
     * Overrides {@see escape_base->create()}
     * @static
     * @param object|array $properties
     * @return escape
     */
    public static function create($properties) {
        return new escape($properties);
    }

    /**
     * Generates a escape object from the database given its id
     * @static
     * @param int $escapeid
     * @return escape
     */
    public static function load($escapeid) {
        global $DB;

        if (!$escape = $DB->get_record('escape', array('id' => $escapeid))) {
    ...
    /**
     * Deletes this escape from the database
     */
    public function delete() {
        ...
        $cm = get_coursemodule_from_instance('escape', $this->properties->id, $this->properties->course);
        $context = context_module::instance($cm->id);

        $this->delete_all_overrides();

        grade_update('mod/escape', $this->properties->course, 'mod', 'escape', $this->properties->id, 0, null, array('deleted'=>1));

        // We must delete the module record after we delete the grade item.
        $DB->delete_records("escape", array("id"=>$this->properties->id));
        $DB->delete_records("escape_pages", array("escapeid"=>$this->properties->id));
        $DB->delete_records("escape_answers", array("escapeid"=>$this->properties->id));
        $DB->delete_records("escape_attempts", array("escapeid"=>$this->properties->id));
        $DB->delete_records("escape_grades", array("escapeid"=>$this->properties->id));
        $DB->delete_records("escape_timer", array("escapeid"=>$this->properties->id));
        $DB->delete_records("escape_branch", array("escapeid"=>$this->properties->id));
        if ($events = $DB->get_records('event', array("modulename"=>'escape', "instance"=>$this->properties->id))) {
    ...
    /**
     * Deletes a escape override from the database and clears any corresponding calendar events
     *
    ...
    public function delete_override($overrideid) {
        ...
        $override = $DB->get_record('escape_overrides', array('id' => $overrideid), '*', MUST_EXIST);
        ...
        $DB->delete_records('escape_overrides', array('id' => $overrideid));
        ...
        $params = array(
            ...
            'context' => context_module::instance($cm->id),
            'other' => array(
                'escapeid' => $override->escapeid
            )
        );
        // Determine which override deleted event to fire.
        if (!empty($override->userid)) {
            $params['relateduserid'] = $override->userid;
            $event = \mod_escape\event\user_override_deleted::create($params);
        } else {
            $params['other']['groupid'] = $override->groupid;
            $event = \mod_escape\event\group_override_deleted::create($params);
        }

        // Trigger the override deleted event.
        $event->add_record_snapshot('escape_overrides', $override);
        $event->trigger();
    ...
    /**
     * Deletes all escape overrides from the database and clears any corresponding calendar events
     */
    public function delete_all_overrides() {
        global $DB;

        $overrides = $DB->get_records('escape_overrides', array('escapeid' => $this->properties->id), 'id');
    ...
    public function is_participant($userid) {
        return is_enrolled($this->get_context(), $userid, 'mod/escape:view', $this->show_only_active_users());
    }
    ...
    /**
     * Updates the escape properties with override information for a user.
     *
     * Algorithm:  For each escape setting, if there is a matching user-specific override,
     ...
     *   escape->extrapasswords will contain an array of strings giving the remaining
     *   passwords.
     *
    ...
    public function update_effective_access($userid) {
        ...
        $override = $DB->get_record('escape_overrides', array('escapeid' => $this->properties->id, 'userid' => $userid));
        ...
        if (!empty($groupings[0])) {
            ...
            $sql = "SELECT * FROM {escape_overrides}
                    WHERE groupid $extra AND escapeid = ?";
        ...
        // Special handling of escape usepassword and password.
    ...
    protected function get_messages() {
        ...
        $messages = array();
        if (!empty($SESSION->escape_messages) && is_array($SESSION->escape_messages) && array_key_exists($this->properties->id, $SESSION->escape_messages)) {
            $messages = $SESSION->escape_messages[$this->properties->id];
            unset($SESSION->escape_messages[$this->properties->id]);
        }
    ...
    public function get_attempts($retries, $correct=false, $pageid=null, $userid=null) {
        global $USER, $DB;
        $params = array("escapeid"=>$this->properties->id, "userid"=>$userid, "retry"=>$retries);
        if ($correct) {
            $params['correct'] = 1;
        }
        if ($pageid !== null) {
        ...
        return $DB->get_records('escape_attempts', $params, 'timeseen ASC');
    }


    /**
     * Get a list of content pages (formerly known as branch tables) viewed in the escape for the given user during an attempt.
     *
     * @param  int $escapeattempt the escape attempt number (also known as retries)
     ...
     * @since  Moodle 3.3
     */
    public function get_content_pages_viewed($escapeattempt, $userid = null, $sort = '', $fields = '*') {
        global $USER, $DB;

        if ($userid === null) {
            $userid = $USER->id;
        }
        $conditions = array("escapeid" => $this->properties->id, "userid" => $userid, "retry" => $escapeattempt);
        return $DB->get_records('escape_branch', $conditions, $sort, $fields);
    }

    /**
     * Returns the first page for the escape or false if there isn't one.
     *
     * This method should be called via the magic method __get();
     * <code>
     * $firstpage = $escape->firstpage;
     * </code>
     *
     * @return escape_page|bool Returns the escape_page specialised object or false
     */
    ...
    /**
     * Returns the last page for the escape or false if there isn't one.
     *
     * This method should be called via the magic method __get();
     * <code>
     * $lastpage = $escape->lastpage;
     * </code>
     *
     * @return escape_page|bool Returns the escape_page specialised object or false
     */
    ...
    /**
     * Returns the id of the first page of this escape. (prevpageid = 0)
     * @return int
     */
    protected function get_firstpageid() {
        ...
        if ($this->firstpageid == null) {
            if (!$this->loadedallpages) {
                $firstpageid = $DB->get_field('escape_pages', 'id', array('escapeid'=>$this->properties->id, 'prevpageid'=>0));
                if (!$firstpageid) {
    ...
    /**
     * Returns the id of the last page of this escape. (nextpageid = 0)
     * @return int
     */
    public function get_lastpageid() {
        ...
        if ($this->lastpageid == null) {
            if (!$this->loadedallpages) {
                $lastpageid = $DB->get_field('escape_pages', 'id', array('escapeid'=>$this->properties->id, 'nextpageid'=>0));
                if (!$lastpageid) {
                    print_error('cannotfindlastpage', 'escape');
                }
    ...
    public function get_next_page($nextpageid) {
        ...
        if ($this->properties->nextpagedefault) {
            ...
            $nretakes = $DB->count_records("escape_grades", array("escapeid" => $this->properties->id, "userid" => $USER->id));
            shuffle($allpages);
            $found = false;
            if ($this->properties->nextpagedefault == ESCAPE_UNSEENPAGE) {
                foreach ($allpages as $nextpage) {
                    if (!$DB->count_records("escape_attempts", array("pageid" => $nextpage->id, "userid" => $USER->id, "retry" => $nretakes))) {
                        $found = true;
                        break;
                    }
                }
            } elseif ($this->properties->nextpagedefault == ESCAPE_UNANSWEREDPAGE) {
                foreach ($allpages as $nextpage) {
                    if (!$DB->count_records("escape_attempts", array('pageid' => $nextpage->id, 'userid' => $USER->id, 'correct' => 1, 'retry' => $nretakes))) {
                        $found = true;
                        break;
                    }
                }
            ...
            if ($found) {
                if ($this->properties->maxpages) {
                    // check number of pages viewed (in the escape)
                    if ($DB->count_records("escape_attempts", array("escapeid" => $this->properties->id, "userid" => $USER->id, "retry" => $nretakes)) >= $this->properties->maxpages) {
                        return ESCAPE_EOL;
                    }
                }
                return $nextpage->id;
    ...
    /**
     * Sets a message against the session for this escape that will displayed next
     * time the escape processes messages
     *
    ...
    public function add_message($message, $class="notifyproblem", $align='center') {
        ...
        if (empty($SESSION->escape_messages) || !is_array($SESSION->escape_messages)) {
            $SESSION->escape_messages = array();
            $SESSION->escape_messages[$this->properties->id] = array();
        } else if (!array_key_exists($this->properties->id, $SESSION->escape_messages)) {
            $SESSION->escape_messages[$this->properties->id] = array();
        }

        $SESSION->escape_messages[$this->properties->id][] = array($message, $class, $align);

        return true;
    }

    /**
     * Check if the escape is accessible at the present time
     * @return bool True if the escape is accessible, false otherwise
     */
    ...
    /**
     * Starts the escape time for the current user
    ...
    public function start_timer() {
        ...
        // Trigger escape started event.
        $event = \mod_escape\event\escape_started::create(array(
        ...
        $USER->startescape[$this->properties->id] = true;

        $timenow = time();
        $startescape = new stdClass;
        $startescape->escapeid = $this->properties->id;
        $startescape->userid = $USER->id;
        $startescape->starttime = $timenow;
        $startescape->escapetime = $timenow;
        if (WS_SERVER) {
            $startescape->timemodifiedoffline = $timenow;
        }
        $DB->insert_record('escape_timer', $startescape);
        if ($this->properties->timelimit) {
            $this->add_message(get_string('timelimitwarning', 'escape', format_time($this->properties->timelimit)), 'center');
        }
        return true;
    }
    ...
    public function update_timer($restart=false, $continue=false, $endreached =false) {
        ...
        if ($restart) {
            if ($continue) {
                ...
                $timer->starttime = time() - ($timer->escapetime - $timer->starttime);

                // Trigger escape resumed event.
                $event = \mod_escape\event\escape_resumed::create(array(
                    'objectid' => $this->properties->id,
                    'context' => context_module::instance($cm->id),
                    'courseid' => $this->properties->course
                ));
                ...
                // Trigger escape restarted event.
                $event = \mod_escape\event\escape_restarted::create(array(
                    'objectid' => $this->properties->id,
                    'context' => context_module::instance($cm->id),
                    'courseid' => $this->properties->course
                ));
        ...
        $timenow = time();
        $timer->escapetime = $timenow;
        if (WS_SERVER) {
            $timer->timemodifiedoffline = $timenow;
        }
        $timer->completed = $endreached;
        $DB->update_record('escape_timer', $timer);
    ...
    public function stop_timer() {
        global $USER, $DB;
        unset($USER->startescape[$this->properties->id]);

        $cm = get_coursemodule_from_instance('escape', $this->properties()->id, $this->properties()->course,
            false, MUST_EXIST);

        // Trigger escape ended event.
        $event = \mod_escape\event\escape_ended::create(array(
            'objectid' => $this->properties()->id,
            'context' => context_module::instance($cm->id),
            'courseid' => $this->properties()->course
        ));
    ...
    /**
     * Checks to see if the escape has pages
     */
    public function has_pages() {
        global $DB;
        $pagecount = $DB->count_records('escape_pages', array('escapeid'=>$this->properties->id));
        return ($pagecount>0);
    }
            ...
            if ($modname) {
                ...
                if ($instancename) {
                    return html_writer::link(new moodle_url('/mod/'.$modname.'/view.php',
                        ...
                        'escape', $instancename), array('class' => 'centerpadded escapebutton standardbutton p-r-1'));
                }
            }
        }
        return '';
     ...
     * escape_page object OR as a generic escape_page.
     ...
     *
     * @param int $pageid
     * @return escape_page A escape_page object or an object that extends it
     */
    public function load_page($pageid) {
        if (!array_key_exists($pageid, $this->pages)) {
            $manager = escape_page_type_manager::get($this);
    ...
    /**
     * Loads ALL of the pages for this escape
     *
     * @return array An array containing all pages from this escape
     */
    ...
    public function duplicate_page($pageid) {
        ...
        $newescapepage = escape_page::create($properties, $this, $context, $PAGE->course->maxbytes);
        $newanswers = $newescapepage->get_answers();
        // Copy over the file areas as well.
        $this->copy_page_files('page_contents', $pageid, $newescapepage->id, $context->id);
        $j = 0;
    ...
    protected function copy_page_files($filearea, $itemid, $newitemid, $contextid) {
        $fs = get_file_storage();
        $files = $fs->get_area_files($contextid, 'mod_escape', $filearea, $itemid);
    ...
    public function jumpto_is_correct($pageid, $jumpto) {
        ...
        if (!$jumpto) {
            // same page
            return false;
        } elseif ($jumpto == ESCAPE_NEXTPAGE) {
            return true;
        } elseif ($jumpto == ESCAPE_UNSEENBRANCHPAGE) {
            return true;
        } elseif ($jumpto == ESCAPE_RANDOMPAGE) {
            return true;
        } elseif ($jumpto == ESCAPE_CLUSTERJUMP) {
            return true;
        } elseif ($jumpto == ESCAPE_EOL) {
            return true;
        }
    ...
    /**
     * Returns the time a user has remaining on this escape
    ...
    /**
     * Interprets ESCAPE_CLUSTERJUMP jumpto value.
     *
     * This will select a page randomly
     * and the page selected will be inbetween a cluster page and end of clutter or end of escape
    ...
    public function cluster_jump($pageid, $userid=null) {
        ...
        // get the number of retakes
        if (!$retakes = $DB->count_records("escape_grades", array("escapeid"=>$this->properties->id, "userid"=>$userid))) {
            $retakes = 0;
        }
        // get all the escape_attempts aka what the user has seen
        $seenpages = array();
        ...
        $clusterpages = array();
        $clusterpages = $this->get_sub_pages_of($pageid, array(ESCAPE_PAGE_ENDOFCLUSTER));
        $unseen = array();
        foreach ($clusterpages as $key=>$cluster) {
            ...
            if ($this->is_sub_page_of_type($cluster->id,
                    array(ESCAPE_PAGE_BRANCHTABLE), array(ESCAPE_PAGE_ENDOFBRANCH, ESCAPE_PAGE_CLUSTER))
                    || $cluster->qtype == ESCAPE_PAGE_ENDOFBRANCH) {
                unset($clusterpages[$key]);
            } else if ($cluster->qtype == ESCAPE_PAGE_BRANCHTABLE) {
                // If branchtable, check to see if any pages inside have been viewed.
                $branchpages = $this->get_sub_pages_of($cluster->id, array(ESCAPE_PAGE_BRANCHTABLE, ESCAPE_PAGE_ENDOFBRANCH));
                $flag = true;
        ...
        if (count($unseen) > 0) {
            ...
            if ($nextpage->qtype == ESCAPE_PAGE_BRANCHTABLE) {
                // if branch table, then pick a random page inside of it
                $branchpages = $this->get_sub_pages_of($nextpage->id, array(ESCAPE_PAGE_BRANCHTABLE, ESCAPE_PAGE_ENDOFBRANCH));
            ...
            if (end($clusterpages)->nextpageid == 0) {
                ...
                while ($clusterendid != 0) { // This condition should not be satisfied... should be an end of cluster page.
                    if ($escapepages[$clusterendid]->qtype == ESCAPE_PAGE_ENDOFCLUSTER) {
                        break;
                    }
                    $clusterendid = $escapepages[$clusterendid]->nextpageid;
                }
                $exitjump = $DB->get_field("escape_answers", "jumpto", array("pageid" => $clusterendid, "escapeid" => $this->properties->id));
                if ($exitjump == ESCAPE_NEXTPAGE) {
                    $exitjump = $escapepages[$clusterendid]->nextpageid;
                }
                if ($exitjump == 0) {
                    return ESCAPE_EOL;
                } else if (in_array($exitjump, array(ESCAPE_EOL, ESCAPE_PREVIOUSPAGE))) {
                    return $exitjump;
                } else {
                    if (!array_key_exists($exitjump, $escapepages)) {
                        $found = false;
                        foreach ($escapepages as $page) {
                            if ($page->id === $clusterendid) {
                                $found = true;
                            } else if ($page->qtype == ESCAPE_PAGE_ENDOFCLUSTER) {
                                $exitjump = $DB->get_field("escape_answers", "jumpto", array("pageid" => $page->id, "escapeid" => $this->properties->id));
                                if ($exitjump == ESCAPE_NEXTPAGE) {
                                    $exitjump = $escapepages[$page->id]->nextpageid;
                                }
                                break;
                            }
                        }
                    ...
                    if ($escapepages[$exitjump]->qtype == ESCAPE_PAGE_CLUSTER) {
     ...
     *
     * @param int $pageid
     * @param array $ends An array of ESCAPE_PAGE_* types that signify an end of
     *               the subtype
     * @return array An array of specialised escape_page objects
     */
    public function get_sub_pages_of($pageid, array $ends) {
        $escapepages = $this->load_all_pages();
        $pageid = $escapepages[$pageid]->nextpageid;  // move to the first page after the branch table
        $pages = array();

        while (true) {
            if ($pageid == 0 || in_array($escapepages[$pageid]->qtype, $ends)) {
                break;
            }
            $pages[] = $escapepages[$pageid];
            $pageid = $escapepages[$pageid]->nextpageid;
        }
    ...
    public function resort_pages($pageid, $after) {
        ...
        if (!array_key_exists($pageid, $pages) || ($after!=0 && !array_key_exists($after, $pages))) {
            print_error('cannotfindpages', 'escape', "$CFG->wwwroot/mod/escape/edit.php?id=$cm->id");
        }
        ...
        if (!empty($pageupdated)) {
            ...
            $event = \mod_escape\event\page_moved::create($eventparams);
            $event->trigger();
        }
    ...
    /**
     * Return the escape context object.
     *
    ...
    /**
     * Set the escape course module object.
     *
    ...
    /**
     * Return the escape course module object.
     *
    ...
    public function get_cm() {
        if ($this->cm == null) {
            $this->cm = get_coursemodule_from_instance('escape', $this->properties->id);
        }
        return $this->cm;
    }

    /**
     * Set the escape course object.
     *
    ...
    /**
     * Return the escape course object.
     *
    ...
    /**
     * Check if the user can manage the escape activity.
     *
     * @return bool true if the user can manage the escape
     * @since  Moodle 3.3
     */
    public function can_manage() {
        return has_capability('mod/escape:manage', $this->get_context());
    }
    ...
    public function get_time_restriction_status() {
        ...
        if (!$this->is_accessible()) {
            if ($this->properties->deadline != 0 && time() > $this->properties->deadline) {
                $status = ['reason' => 'escapeclosed', 'time' => $this->properties->deadline];
            } else {
                $status = ['reason' => 'escapeopen', 'time' => $this->properties->available];
            }
    ...
    public function get_dependencies_restriction_status() {
        ...
        if ($dependentescape = $DB->get_record('escape', array('id' => $this->properties->dependency))) {
            // Escape exists, so we can proceed.
            ...
            if ($conditions->timespent) {
                $timespent = false;
                if ($attempttimes = $DB->get_records('escape_timer', array("userid" => $USER->id, "escapeid" => $dependentescape->id))) {
                    // Go through all the times and test to see if any of them satisfy the condition.
                    foreach ($attempttimes as $attempttime) {
                        $duration = $attempttime->escapetime - $attempttime->starttime;
                ...
                if (!$timespent) {
                    $errors[] = get_string('timespenterror', 'escape', $conditions->timespent);
                }
            }
            // Check for the gradebetterthan condition.
            if ($conditions->gradebetterthan) {
                $gradebetterthan = false;
                if ($studentgrades = $DB->get_records('escape_grades', array("userid" => $USER->id, "escapeid" => $dependentescape->id))) {
                ...
                if (!$gradebetterthan) {
                    $errors[] = get_string('gradebetterthanerror', 'escape', $conditions->gradebetterthan);
                }
            }
            // Check for the completed condition.
            if ($conditions->completed) {
                if (!$DB->count_records('escape_grades', array('userid' => $USER->id, 'escapeid' => $dependentescape->id))) {
                    $errors[] = get_string('completederror', 'escape');
                }
            }
            if (!empty($errors)) {
                return (object) ['errors' => $errors, 'dependentescape' => $dependentescape];
            }
        }
        return false;
    }
    ...
    /**
     * Check if the escape is in review mode. (The user already finished it and retakes are not allowed).
     *
    ...
    public function is_in_review_mode() {
        ...
        $userhasgrade = $DB->count_records("escape_grades", array("escapeid" => $this->properties->id, "userid" => $USER->id));
     ...
     *
     * @param int $retriescount the number of retries for the escape (the last retry number).
     * @return mixed false if the user didn't see the escape or the last page id
     */
    public function get_last_page_seen($retriescount) {
        ...
        if (!empty($allattempts)) {
            ...
            $jumpto = $DB->get_field('escape_answers', 'jumpto', array('id' => $attempt->answerid));
            ...
            } else if ($jumpto == ESCAPE_NEXTPAGE) {
                $lastpageseen = $this->get_next_page($attemptpage->nextpageid);
            } else if ($jumpto == ESCAPE_CLUSTERJUMP) {
    ...
    /**
     * Return the number of retries in a escape for a given user.
     *
     * @param  int $userid the user id
    ...
    public function count_user_retries($userid) {
        ...
        return $DB->count_records('escape_grades', array("escapeid" => $this->properties->id, "userid" => $userid));
    }
    ...
    public function left_during_timed_session($retriescount) {
        ...
        $conditions = array('escapeid' => $this->properties->id, 'userid' => $USER->id, 'retry' => $retriescount);
        return $DB->count_records('escape_attempts', $conditions) > 0 || $DB->count_records('escape_branch', $conditions) > 0;
    }
    ...
    public function set_module_viewed() {
        ...
        $event = \mod_escape\event\course_module_viewed::create(array(
            'objectid' => $this->properties->id,
     ...
     * @return array    list of timers for the given user in the escape
     * @since  Moodle 3.3
     */
    public function get_user_timers($userid = null, $sort = '', $fields = '*', $limitfrom = 0, $limitnum = 0) {
        ...
        $params = array('escapeid' => $this->properties->id, 'userid' => $userid);
        return $DB->get_records('escape_timer', $params, $sort, $fields, $limitfrom, $limitnum);
    }

    /**
     * Check if the user is out of time in a timed escape.
     *
    ...
    public function check_time($timer) {
        if ($this->properties->timelimit) {
            ...
            if ($timeleft <= 0) {
                // Out of time.
                $this->add_message(get_string('eolstudentoutoftime', 'escape'));
                return false;
            } else if ($timeleft < 60) {
                // One minute warning.
                $this->add_message(get_string('studentoneminwarning', 'escape'));
            }
        }
        return true;
    }
     ...
     *
     * @param escape_page $page page object
     * @param reviewmode $bool whether we are in review mode or not
     * @since  Moodle 3.3
     */
    public function add_messages_on_page_view(escape_page $page, $reviewmode) {
        global $DB, $USER;

        if (!$this->can_manage()) {
            if ($page->qtype == ESCAPE_PAGE_BRANCHTABLE && $this->properties->minquestions) {
                // Tell student how many questions they have seen, how many are required and their grade.
                $ntries = $DB->count_records("escape_grades", array("escapeid" => $this->properties->id, "userid" => $USER->id));
                $gradeinfo = escape_grade($this, $ntries);
                if ($gradeinfo->attempts) {
                    if ($gradeinfo->nquestions < $this->properties->minquestions) {
                        ...
                        $this->add_message(get_string('numberofpagesviewednotice', 'escape', $a));
                    }

                    if (!$reviewmode && $this->properties->ongoing) {
                        $this->add_message(get_string("numberofcorrectanswers", "escape", $gradeinfo->earned), 'notify');
                        if ($this->properties->grade != GRADE_TYPE_NONE) {
                            ...
                            $this->add_message(get_string('yourcurrentgradeisoutof', 'escape', $a), 'notify');
                        }
                    }
                }
            }
            ...
            if ($this->properties->timelimit) {
                $this->add_message(get_string('teachertimerwarning', 'escape'));
            }
            if (escape_display_teacher_warning($this)) {
                ...
                $warningvars->cluster = get_string('clusterjump', 'escape');
                $warningvars->unseen = get_string('unseenpageinbranch', 'escape');
                $this->add_message(get_string('teacherjumpwarning', 'escape', $warningvars));
            }
        }
    }

    /**
     * Get the ongoing score message for the user (depending on the user permission and escape settings).
     *
    ...
    public function get_ongoing_score_message() {
        ...
        if (has_capability('mod/escape:manage', $context)) {
            return get_string('teacherongoingwarning', 'escape');
        } else {
            $ntries = $DB->count_records("escape_grades", array("escapeid" => $this->properties->id, "userid" => $USER->id));
            ...
            if ($this->properties->custom) {
                ...
                return get_string("ongoingcustom", "escape", $a);
            } else {
                $a->correct = $gradeinfo->earned;
                $a->viewed = $gradeinfo->attempts;
                return get_string("ongoingnormal", "escape", $a);
            }
        }
    }

    /**
     * Calculate the progress of the current user in the escape.
     *
    ...
    public function calculate_progress() {
        ...
        if (!$ntries = $DB->count_records("escape_grades", array("escapeid" => $this->properties->id, "userid" => $USER->id))) {
     ...
     * @param  mod_escape_renderer $escapeoutput the escape output rendered
     ...
     * @since  Moodle 3.3
     */
    public function prepare_page_and_contents($pageid, $escapeoutput, $reviewmode, $redirect = true) {
        ...
        if (isset($page->escape->pages[$page->id]->typeidstring)) {
            if ($page->escape->pages[$page->id]->typeidstring == "clickingpicture") {
                $PAGE->requires->css('/mod/escape/style/custom_style_form.css');
                $PAGE->requires->js_call_amd('mod_escape/img_form_manager', 'init',array());
            }
        }
        // Add different informative messages to the given page.
        ...
        if (is_array($page->answers) && count($page->answers) > 0) {
            ...
            if (isset($USER->modattempts[$this->properties->id])) {
                ...
                if (!$attempts = $this->get_attempts($retries - 1, false, $page->id)) {
                    throw new moodle_exception('cannotfindpreattempt', 'escape');
                }
                $attempt = end($attempts);
            ...
            $escapecontent = $escapeoutput->display_page($this, $page, $attempt);
        } else {
            require_once($CFG->dirroot . '/mod/escape/view_form.php');
            $data = new stdClass;
            ...
            $mform = new escape_page_without_answers($CFG->wwwroot.'/mod/escape/continue.php', $customdata);
            $mform->set_data($data);
            ob_start();
            $mform->display();
            $escapecontent = ob_get_contents();
            ob_end_clean();
        }

        return array($page->id, $page, $escapecontent);
    }

    /**
     * This returns a real page id to jump to (or ESCAPE_EOL) after processing page responses.
     *
     * @param  escape_page $page      escape page
     * @param  int         $newpageid the new page id
     * @return int the real page to jump to (or end of escape)
     * @since  Moodle 3.3
     */
    public function calculate_new_page_on_jump(escape_page $page, $newpageid) {
        global $USER, $DB;
        ...
        if (isset($USER->modattempts[$this->properties->id])) {
            ...
            if ($USER->modattempts[$this->properties->id]->pageid == $page->id && $page->nextpageid == 0) {
                ...
                $attempts = $DB->get_records("escape_attempts", array("escapeid" => $this->properties->id, "userid" => $USER->id, "retry" => $nretakes), "timeseen", "id, pageid");
                $found = false;
                $temppageid = 0;
        ...
        } else if ($newpageid != ESCAPE_CLUSTERJUMP && $page->id != 0 && $newpageid > 0) {
        ...
        } else if ($newpageid == ESCAPE_UNSEENBRANCHPAGE) {
            if ($canmanage) {
                ...
                $newpageid = escape_unseen_question_jump($this, $USER->id, $page->id);
            }
        } else if ($newpageid == ESCAPE_PREVIOUSPAGE) {
            $newpageid = $page->prevpageid;
        } else if ($newpageid == ESCAPE_RANDOMPAGE) {
            $newpageid = escape_random_question_jump($this, $page->id);
        } else if ($newpageid == ESCAPE_CLUSTERJUMP) {
            if ($canmanage) {
            ...
            $newpageid = $page->id;
        } else if ($newpageid == ESCAPE_NEXTPAGE) {
     ...
     * @since  Moodle 3.3
     */
    public function process_page_responses(escape_page $page) {
        $context = $this->get_context();
     ...
     * @since  Moodle 3.3
     */
    public function add_messages_on_page_process(escape_page $page, $result, $reviewmode) {
        ...
        if ($result->attemptsremaining != 0 && $this->properties->review && !$reviewmode) {
            $this->add_message(get_string('attemptsremaining', 'escape', $result->attemptsremaining));
        }
    }

    /**
     * Process and return all the information for the end of escape page.
     *
    ...
    public function process_eol_page($outoftime) {
        ...
        if ($this->properties->custom && !$canmanage) {
            ...
            // number specified in the minimum questions setting - which is done in escape_grade().
            // Get the number of answers given.
            if ($gradeinfo->nquestions < $this->properties->minquestions) {
                $data->gradeescape = false;
                $a = new stdClass;
                $a->nquestions = $gradeinfo->nquestions;
                $a->minquestions = $this->properties->minquestions;
                $this->add_message(get_string('numberofpagesviewednotice', 'escape', $a));
        ...
        if (!$canmanage) {
            if ($data->gradeescape) {
                    ...
                    $duration = $DB->get_field_sql(
                        "SELECT SUM(escapetime - starttime)
                                      ...
                                      WHERE escapeid = :escapeid
                                        AND userid = :userid",
                        array('userid' => $USER->id, 'escapeid' => $this->properties->id));
                    if (!$duration) {
                        $duration = 0;
                    }

                    // If student has not spend enough time in the escape, display a message.
                ...
                if ($gradeinfo->attempts) {
                    ...
                    $grade = new stdClass();
                    $grade->escapeid = $this->properties->id;
                    ...
                    if (isset($USER->modattempts[$this->properties->id])) { // If reviewing, make sure update old grade record.
                        if (!$grades = $DB->get_records("escape_grades",
                            array("escapeid" => $this->properties->id, "userid" => $USER->id), "completed DESC", '*', 0, 1)) {
                            throw new moodle_exception('cannotfindgrade', 'escape');
                        }
                        $oldgrade = array_shift($grades);
                        $grade->id = $oldgrade->id;
                        $DB->update_record("escape_grades", $grade);
                    } else {
                        $newgradeid = $DB->insert_record("escape_grades", $grade);
                    }
                } else {
                    if ($this->properties->timelimit) {
                        if ($outoftime == 'normal') {
                ...
                escape_update_grades($this, $USER->id);
        ...
        if ($this->properties->modattempts && !$canmanage) {
            ...
            if (!$attempts = $this->get_attempts($ntries)) {
                $attempts = array();
                $url = new moodle_url('/mod/escape/view.php', array('id' => $cm->id));
            } else {
                ...
                // This will help to make sure that the student can leave the escape via pushing the continue button.
                $lastattempt = end($attempts);
                $USER->modattempts[$this->properties->id] = $lastattempt->pageid;

                $url = new moodle_url('/mod/escape/view.php', array('id' => $cm->id, 'pageid' => $pageid));
            }
            $data->reviewescape = $url->out(false);
...
/**
 * Abstract class to provide a core functions to the all escape classes
 *
 * This class should be abstracted by ALL classes with the escape module to ensure
...
/**
 * Abstract class representation of a page associated with a escape.
 *
 * This class should MUST be extended by all specialised page types defined in
 * mod/escape/pagetypes/.
 ...
 *
 * Database properties
 * @property int $id The id of this escape page
 * @property int $escapeid The id of the escape this page belongs to
 ...
 * @property-read escape $escape The escape this page belongs to
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class escape_page extends escape_base {

    /**
     * A reference to the escape this page belongs to
     * @var escape
     */
    protected $escape = null;
    /**
     * Contains the answers to this escape_page once loaded
     * @var null|array
     */
    ...
    /**
     * This method gets called to display the page to the user taking the escape
     * @abstract
    ...
    /**
     * Creates a new escape_page within the database and returns the correct pagetype
     * object to use to interact with the new escape
     *
     * @final
     * @static
     * @param object $properties
     * @param escape $escape
     * @return escape_page Specialised object that extends escape_page
     */
    final public static function create($properties, escape $escape, $context, $maxbytes) {
        global $DB;
        ...
        $newpage->location = $properties->location;
        if ($properties->pageid) {
            $prevpage = $DB->get_record("escape_pages", array("id" => $properties->pageid), 'id, nextpageid');
            if (!$prevpage) {
                print_error('cannotfindpages', 'escape');
            }
            ...
            $nextpage = $DB->get_record('escape_pages', array('escapeid'=>$escape->id, 'prevpageid'=>0), 'id');
            if ($nextpage) {
        ...
        $newpage->id = $DB->insert_record("escape_pages", $newpage);

        $editor = new stdClass;
        $editor->id = $newpage->id;
        $editor->contents_editor = $properties->contents_editor;
        $editor = file_postupdate_standard_editor($editor, 'contents', array('noclean'=>true, 'maxfiles'=>EDITOR_UNLIMITED_FILES, 'maxbytes'=>$maxbytes), $context, 'mod_escape', 'page_contents', $editor->id);
        $DB->update_record("escape_pages", $editor);

        if ($newpage->prevpageid > 0) {
            $DB->set_field("escape_pages", "nextpageid", $newpage->id, array("id" => $newpage->prevpageid));
        }
        if ($newpage->nextpageid > 0) {
            $DB->set_field("escape_pages", "prevpageid", $newpage->id, array("id" => $newpage->nextpageid));
        }

        $page = escape_page::load($newpage, $escape);
        ...
        $event = \mod_escape\event\page_created::create($eventparams);
        $snapshot = clone($newpage);
        $snapshot->timemodified = 0;
        $event->add_record_snapshot('escape_pages', $snapshot);
        $event->trigger();

        $escape->add_message(get_string('insertedpage', 'escape').': '.format_string($newpage->title, true), 'notifysuccess');
     ...
     * specialised object that extends escape_page
     *
     * @final
     * @static
     * @param int $id
     * @param escape $escape
     * @return escape_page Specialised escape_page object
     */
    final public static function load($id, escape $escape) {
        global $DB;

        if (is_object($id) && !empty($id->qtype)) {
            $page = $id;
        } else {
            $page = $DB->get_record("escape_pages", array("id" => $id));
            if (!$page) {
                print_error('cannotfindpages', 'escape');
            }
        }
        ...
        $class = 'escape_page_type_'.$manager->get_page_type_idstring($page->qtype);
        if (!class_exists($class)) {
            $class = 'escape_page';
        }

        return new $class($page, $escape);
    }

    /**
     * Deletes a escape_page from the database as well as any associated records.
     * @final
     * @return bool
     */
    final public function delete() {
        ...
        $cm = get_coursemodule_from_instance('escape', $this->escape->id, $this->escape->course);
        $context = context_module::instance($cm->id);

        // Delete files associated with attempts.
        $fs = get_file_storage();
        if ($attempts = $DB->get_records('escape_attempts', array("pageid" => $this->properties->id))) {
            foreach ($attempts as $attempt) {
                $fs->delete_area_files($context->id, 'mod_escape', 'essay_responses', $attempt->id);
            }
        }

        // Then delete all the associated records...
        $DB->delete_records("escape_attempts", array("pageid" => $this->properties->id));

        $DB->delete_records("escape_branch", array("pageid" => $this->properties->id));

        // Delete files related to answers and responses.
        if ($answers = $DB->get_records("escape_answers", array("pageid" => $this->properties->id))) {
            foreach ($answers as $answer) {
                $fs->delete_area_files($context->id, 'mod_escape', 'page_answers', $answer->id);
                $fs->delete_area_files($context->id, 'mod_escape', 'page_responses', $answer->id);
            }
        }

        // ...now delete the answers...
        $DB->delete_records("escape_answers", array("pageid" => $this->properties->id));
        // ..and the page itself
        $DB->delete_records("escape_pages", array("id" => $this->properties->id));
        ...
        $event = \mod_escape\event\page_deleted::create($eventparams);
        $event->add_record_snapshot('escape_pages', $this->properties);
        $event->trigger();

        // Delete files associated with this page.
        $fs->delete_area_files($context->id, 'mod_escape', 'page_contents', $this->properties->id);
        ...
        if (!$this->properties->prevpageid && !$this->properties->nextpageid) {
            ...
            $page = $this->escape->load_page($this->properties->nextpageid);
            $page->move(null, 0);
        } elseif (!$this->properties->nextpageid) {
            // this is the last page...
            $page = $this->escape->load_page($this->properties->prevpageid);
            $page->move(0);
        } else {
            // page is in the middle...
            $prevpage = $this->escape->load_page($this->properties->prevpageid);
            $nextpage = $this->escape->load_page($this->properties->nextpageid);
    ...
    final public function move($nextpageid=null, $prevpageid=null) {
        ...
        $DB->update_record('escape_pages', $obj);
    }
    ...
    final public function get_answers() {
        ...
        if ($this->answers === null) {
            $this->answers = array();
            $answers = $DB->get_records('escape_answers', array('pageid'=>$this->properties->id, 'escapeid'=>$this->escape->id), 'id');
            if (!$answers) {
                // It is possible that a escape upgraded from Moodle 1.9 still
                // contains questions without any answers [MDL-25632].
                // debugging(get_string('cannotfindanswer', 'escape'));
                return array();
            }
            foreach ($answers as $answer) {
                $this->answers[count($this->answers)] = new escape_page_answer($answer);
            }
        }
        return $this->answers;
    }
    ...
    /**
     * Returns the escape this page is associated with
     * @final
     * @return escape
     */
    final protected function get_escape() {
        return $this->escape;
    }
    ...
    final public function record_attempt($context) {
        ...
        if ($result->noanswer) {
            ...
            $result->feedback  = get_string('noanswer', 'escape');
        } else {
            if (!has_capability('mod/escape:manage', $context)) {
                $nretakes = $DB->count_records("escape_grades", array("escapeid"=>$this->escape->id, "userid"=>$USER->id));

                // Get the number of attempts that have been made on this question for this student and retake,
                $nattempts = $DB->count_records('escape_attempts', array('escapeid' => $this->escape->id,
                ...
                if ($nattempts >= $this->escape->maxattempts) {
                    $result->maxattemptsreached = true;
                    $result->feedback = get_string('maximumnumberofattemptsreached', 'escape');
                    $result->newpageid = $this->escape->get_next_page($this->properties->nextpageid);
                    return $result;
                }

                // record student's attempt
                $attempt = new stdClass;
                $attempt->escapeid = $this->escape->id;
                ...
                // Only insert a record if we are not reviewing the escape.
                if (!$userisreviewing) {
                    if ($this->escape->retake || (!$this->escape->retake && $nretakes == 0)) {
                        $attempt->id = $DB->insert_record("escape_attempts", $attempt);
                        ...
                        $event = \mod_escape\event\question_answered::create($eventparams);
                ...
                // "number of attempts remaining" message if $this->escape->maxattempts > 1
                ...
                if (!$result->correctanswer && ($result->newpageid == 0)) {
                    ...
                    if ($nattempts >= $this->escape->maxattempts) {
                        if ($this->escape->maxattempts > 1) { // don't bother with message if only one attempt
                            $result->maxattemptsreached = true;
                        }
                        $result->newpageid = ESCAPE_NEXTPAGE;
                    } else if ($this->escape->maxattempts > 1) { // don't bother with message if only one attempt
                        $result->attemptsremaining = $this->escape->maxattempts - $nattempts;
                    }
                }
            }
            ...
            if (empty($result->response)) {
                if (!$this->escape->feedback && !$result->noanswer && !($this->escape->review & !$result->correctanswer && !$result->isessayquestion)) {
                    // These conditions have been met:
                    //  1. The escape manager has not supplied feedback to the student
                    ...
                    $result->response = get_string('defaultessayresponse', 'escape');
                } else if ($result->correctanswer) {
                    $result->response = get_string('thatsthecorrectanswer', 'escape');
                } else {
                    $result->response = get_string('thatsthewronganswer', 'escape');
                }
            }

            if ($result->response) {
                if ($this->escape->review && !$result->correctanswer && !$result->isessayquestion) {
                    $nretakes = $DB->count_records("escape_grades", array("escapeid"=>$this->escape->id, "userid"=>$USER->id));
                    $qattempts = $DB->count_records("escape_attempts", array("userid"=>$USER->id, "retry"=>$nretakes, "pageid"=>$this->properties->id));
                    if ($qattempts == 1) {
                        $result->feedback = $OUTPUT->box(get_string("firstwrong", "escape"), 'feedback');
                    } else {
                        if (!$result->maxattemptsreached) {
                            $result->feedback = $OUTPUT->box(get_string("secondpluswrong", "escape"), 'feedback');
                        } else {
                            $result->feedback = $OUTPUT->box(get_string("finalwrong", "escape"), 'feedback');
                        }
                    }
                } else {
                    $result->feedback = '';
                ...
                $result->feedback .= '<div class="correctanswer generalbox"><em>'
                        . get_string("youranswer", "escape").'</em> : <div class="studentanswer m-t-2 m-b-2">';
                ...
                if ($this->properties->qoption) {
                    ...
                    foreach ($studentanswerresponse as $answer => $response) {
                        ...
                        if (!empty(trim($response))) {
                            ...
                            $studentresponsecontent = html_writer::div('<em>' . get_string("response", "escape") .
    ...
    private function format_response($response, $context, $responseformat, $answerid, $options) {
        ...
        $convertstudentresponse = file_rewrite_pluginfile_urls($response, 'pluginfile.php',
            $context->id, 'mod_escape', 'page_responses', $answerid);
    ...
    final protected function get_jump_name($jumpto) {
        ...
        if (!array_key_exists($jumpto, $jumpnames)) {
            if ($jumpto == ESCAPE_THISPAGE) {
                $jumptitle = get_string('thispage', 'escape');
            } elseif ($jumpto == ESCAPE_NEXTPAGE) {
                $jumptitle = get_string('nextpage', 'escape');
            } elseif ($jumpto == ESCAPE_EOL) {
                $jumptitle = get_string('endofescape', 'escape');
            } elseif ($jumpto == ESCAPE_UNSEENBRANCHPAGE) {
                $jumptitle = get_string('unseenpageinbranch', 'escape');
            } elseif ($jumpto == ESCAPE_PREVIOUSPAGE) {
                $jumptitle = get_string('previouspage', 'escape');
            } elseif ($jumpto == ESCAPE_RANDOMPAGE) {
                $jumptitle = get_string('randompageinbranch', 'escape');
            } elseif ($jumpto == ESCAPE_RANDOMBRANCH) {
                $jumptitle = get_string('randombranch', 'escape');
            } elseif ($jumpto == ESCAPE_CLUSTERJUMP) {
                $jumptitle = get_string('clusterjump', 'escape');
            } else {
                if (!$jumptitle = $DB->get_field('escape_pages', 'title', array('id' => $jumpto))) {
                    $jumptitle = '<strong>'.get_string('notdefined', 'escape').'</strong>';
                }
            }
     ...
     * @param escape $escape
     */
    public function __construct($properties, escape $escape) {
        parent::__construct($properties);
        $this->escape = $escape;
    }
    ...
    public function save_answers_files($context, $maxbytes, &$answer, $answereditor = '', $responseeditor = '') {
        ...
        if (isset($answereditor['itemid'])) {
            $answer->answer = file_save_draft_area_files($answereditor['itemid'],
                    $context->id, 'mod_escape', 'page_answers', $answer->id,
                    array('noclean' => true, 'maxfiles' => EDITOR_UNLIMITED_FILES, 'maxbytes' => $maxbytes),
                    $answer->answer, null);
            $DB->set_field('escape_answers', 'answer', $answer->answer, array('id' => $answer->id));
        }
        if (isset($responseeditor['itemid'])) {
            $answer->response = file_save_draft_area_files($responseeditor['itemid'],
                    $context->id, 'mod_escape', 'page_responses', $answer->id,
                    array('noclean' => true, 'maxfiles' => EDITOR_UNLIMITED_FILES, 'maxbytes' => $maxbytes),
                    $answer->response, null);
            $DB->set_field('escape_answers', 'response', $answer->response, array('id' => $answer->id));
        }
    }
    ...
    public static function rewrite_answers_urls($answer, $rewriteanswer = true) {
        ...
        if ($rewriteanswer) {
            $answer->answer = file_rewrite_pluginfile_urls($answer->answer, 'pluginfile.php', $context->id,
                    'mod_escape', 'page_answers', $answer->id);
        }
        $answer->response = file_rewrite_pluginfile_urls($answer->response, 'pluginfile.php', $context->id,
                'mod_escape', 'page_responses', $answer->id);

        return $answer;
    }

    /**
     * Updates a escape page and its answers within the database
     *
     * @param object $properties
     * @return bool
     */
    public function update($properties, $context = null, $maxbytes = null) {
        ...
        $properties = file_postupdate_standard_editor($properties, 'contents', array('noclean'=>true, 'maxfiles'=>EDITOR_UNLIMITED_FILES, 'maxbytes'=>$maxbytes), $context, 'mod_escape', 'page_contents', $properties->id);
        ...
        if ($this->type == self::TYPE_STRUCTURE && $this->get_typeid() != ESCAPE_PAGE_BRANCHTABLE && $this->get_typeid() != ESCAPE_PAGE_CLICKINGPICTURE) {
            // These page types have only one answer to save the jump and score.
            if (count($answers) > 1) {
                $answer = array_shift($answers);
                foreach ($answers as $a) {
                    $DB->delete_records('escape_answers', array('id' => $a->id));
                }
            } else if (count($answers) == 1) {
                $answer = array_shift($answers);
            } else {
                $answer = new stdClass;
                $answer->escapeid = $properties->escapeid;
            ...
            if (!empty($answer->id)) {
                $DB->update_record("escape_answers", $answer->properties());
            } else {
                $DB->insert_record("escape_answers", $answer);
            }
        } else {
    ...
    public function is_unanswered($nretakes) {
        global $DB, $USER;
        if (!$DB->count_records("escape_attempts", array('pageid'=>$this->properties->id, 'userid'=>$USER->id, 'correct'=>1, 'retry'=>$nretakes))) {
            return true;
        }
        return false;
    }
    ...
    /**
     * Creates answers within the database for this escape_page. Usually only ever
    ...
    public function create_answers($properties) {
        ...
        for ($i = 0; $i < $this->escape->maxanswers; $i++) {
            ...
            if (isset($answer->answer) && $answer->answer != '') {
                ...
                $answer->id = $DB->insert_record("escape_answers", $answer);
                if (isset($properties->response_editor[$i])) {
                ...
                $answers[$answer->id] = new escape_page_answer($answer);
            } else {
                break;
            }
        }
     ...
     * of answers permitted by the escape.
     *
     * @param int $default
     * @return int
     */
    ...
    /**
     * Returns the properties of this escape page as an object
     * @return stdClass;
     */
    public function properties() {
     ...
     * @return array
     */
    public static function get_jumptooptions($pageid, escape $escape) {
        global $DB;
        $jump = array();
        ...
        $pages = $escape->load_all_pages();
        if ($pages[$pageid]->qtype == ESCAPE_PAGE_BRANCHTABLE || $escape->is_sub_page_of_type($pageid, array(ESCAPE_PAGE_BRANCHTABLE), array(ESCAPE_PAGE_ENDOFBRANCH, ESCAPE_PAGE_CLUSTER))) {
        ...
        if($pages[$pageid]->qtype == ESCAPE_PAGE_CLUSTER || $escape->is_sub_page_of_type($pageid, array(ESCAPE_PAGE_CLUSTER), array(ESCAPE_PAGE_ENDOFCLUSTER))) {
            $jump[ESCAPE_CLUSTERJUMP] = get_string("clusterjump", "escape");
        }
        if (!optional_param('firstpage', 0, PARAM_INT)) {
            $apageid = $DB->get_field("escape_pages", "id", array("escapeid" => $escape->id, "prevpageid" => 0));
            while (true) {
                if ($apageid) {
    ...
    public function get_contents() {
        ...
        if (!empty($this->properties->contents)) {
            ...
            $context = context_module::instance($PAGE->cm->id);
            $contents = file_rewrite_pluginfile_urls($this->properties->contents, 'pluginfile.php', $context->id, 'mod_escape',
    ...
    public function get_files($includedirs = true, $updatedsince = 0) {
        $fs = get_file_storage();
        return $fs->get_area_files($this->escape->context->id, 'mod_escape', 'page_contents', $this->properties->id,
 ...
 * @property int $escapeid The ID of the escape this answer belongs to
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class escape_page_answer extends escape_base {

    /**
     * Loads an page answer from the DB
     *
     * @param int $id
     * @return escape_page_answer
     */
    public static function load($id) {
        global $DB;
        $answer = $DB->get_record("escape_answers", array("id" => $id));
        return new escape_page_answer($answer);
    }
     ...
     * @return array
     */
    public static function create($properties, escape_page $page) {
    ...
    public function get_files($includedirs = true, $updatedsince = 0) {
        ...
        $escape = escape::load($this->properties->escapeid);
        $fs = get_file_storage();
        $answerfiles = $fs->get_area_files($escape->context->id, 'mod_escape', 'page_answers', $this->properties->id,
                                            'itemid, filepath, filename', $includedirs, $updatedsince);
        $responsefiles = $fs->get_area_files($escape->context->id, 'mod_escape', 'page_responses', $this->properties->id,
                                            'itemid, filepath, filename', $includedirs, $updatedsince);
 ...
 * different page types located in mod/escape/pagetypes.
 *
 * @copyright  2009 Sam Hemelryk
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class escape_page_type_manager {
    ...
    /**
     * Retrieves the escape page type manager object
     *
     * If the object hasn't yet been created it is created here.
     *
     * @staticvar escape_page_type_manager $pagetypemanager
     * @param escape $escape
     * @return escape_page_type_manager
     */
    public static function get(escape $escape) {
        static $pagetypemanager;
        if (!($pagetypemanager instanceof escape_page_type_manager)) {
            $pagetypemanager = new escape_page_type_manager();
            $pagetypemanager->load_escape_types($escape);
        }
        return $pagetypemanager;
    }

    /**
     * Finds and loads all escape page types in mod/escape/pagetypes
     *
     * @param escape $escape
     */
    public function load_escape_types(escape $escape) {
        global $CFG;
        $basedir = $CFG->dirroot.'/mod/escape/pagetypes/';
        $dir = dir($basedir);
        while (false !== ($entry = $dir->read())) {
            ...
            $class = 'escape_page_type_'.strtok($entry,'.');
            if (class_exists($class)) {
                $pagetype = new $class(new stdClass, $escape);
    ...
    /**
     * Loads a page for the provided escape given it's id
     *
     * This function loads a page from the escape when given both the escape it belongs
     * to as well as the page's id.
     * If the page doesn't exist an error is thrown
     *
     * @param int $pageid The id of the page to load
     * @param escape $escape The escape the page belongs to
     * @return escape_page A class that extends escape_page
     */
    public function load_page($pageid, escape $escape) {
        global $DB;
        if (!($page =$DB->get_record('escape_pages', array('id'=>$pageid, 'escapeid'=>$escape->id)))) {
            print_error('cannotfindpages', 'escape');
        }
        $pagetype = get_class($this->types[$page->qtype]);
        $page = new $pagetype($page, $escape);
        return $page;
    }
    ...
    protected function check_page_order($page1, $page2) {
        ...
        if (empty($page1)) {
            if ($page2->prevpageid != 0) {
                ...
                $page2->prevpageid = 0;
                $DB->set_field("escape_pages", "prevpageid", 0, array("id" => $page2->id));
            }
        } else if (empty($page2)) {
            if ($page1->nextpageid != 0) {
                debugging("***nextpageid of page " . $page1->id . " set to 0***");
                $page1->nextpageid = 0;
                $DB->set_field("escape_pages", "nextpageid", 0, array("id" => $page1->id));
            }
        } else {
            if ($page1->nextpageid != $page2->id) {
                debugging("***nextpageid of page " . $page1->id . " set to " . $page2->id . "***");
                $page1->nextpageid = $page2->id;
                $DB->set_field("escape_pages", "nextpageid", $page2->id, array("id" => $page1->id));
            }
            if ($page2->prevpageid != $page1->id) {
                debugging("***prevpageid of page " . $page2->id . " set to " . $page1->id . "***");
                $page2->prevpageid = $page1->id;
                $DB->set_field("escape_pages", "prevpageid", $page1->id, array("id" => $page2->id));
            }
        }
    }

    /**
     * This function loads ALL pages that belong to the escape.
     *
     * @param escape $escape
     * @return array An array of escape_page_type_*
     */
    public function load_all_pages(escape $escape) {
        global $DB;
        if (!($pages =$DB->get_records('escape_pages', array('escapeid'=>$escape->id)))) {
            return array(); // Records returned empty.
        }
        foreach ($pages as $key=>$page) {
            $pagetype = get_class($this->types[$page->qtype]);
            $pages[$key] = new $pagetype($page, $escape);
        }
     ...
     * @return escape_add_page_form_base
     */
    public function get_page_form($type, $arguments) {
        $class = 'escape_add_page_form_'.$this->get_page_type_idstring($type);
        if (!class_exists($class) || get_parent_class($class)!=='escape_add_page_form_base') {
            debugging('Escape page type unknown class requested '.$class, DEBUG_DEVELOPER);
            $class = 'escape_add_page_form_selection';
        } else if ($class === 'escape_add_page_form_unknown') {
            $class = 'escape_add_page_form_selection';
        }
