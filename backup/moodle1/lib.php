...
/**
 * Escape conversion handler
 */
class moodle1_mod_escape_handler extends moodle1_mod_handler {
     ...
     * defined.
     *
     * Note that the path /MOODLE_BACKUP/COURSE/MODULES/MOD/ESCAPE does not
    ...
    public function get_paths() {
        return array(
            new convert_path(
                'escape', '/MOODLE_BACKUP/COURSE/MODULES/MOD/ESCAPE',
                array(
                    'renamefields' => array(
            ...
            new convert_path(
                'escape_page', '/MOODLE_BACKUP/COURSE/MODULES/MOD/ESCAPE/PAGES/PAGE',
                array(
                    'newfields' => array(
                        'contentsformat' => FORMAT_MOODLE,
                        'nextpageid' => 0, //set to default to the next sequencial page in process_escape_page()
                        'prevpageid' => 0
                    ),
                )
            ),
            new convert_path(
                'escape_pages', '/MOODLE_BACKUP/COURSE/MODULES/MOD/ESCAPE/PAGES'
            ),
            new convert_path(
                'escape_answer', '/MOODLE_BACKUP/COURSE/MODULES/MOD/ESCAPE/PAGES/PAGE/ANSWERS/ANSWER',
                array(
                    'newfields' => array(
                        'answerformat' => 0,
                        'responseformat' => 0,
    ...
    /**
     * This is executed every time we have one /MOODLE_BACKUP/COURSE/MODULES/MOD/ESCAPE
     * data available
     */
    public function process_escape($data) {
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_escape');
        ...
        if (!empty($data['mediafile']) and strpos($data['mediafile'], '://') === false) {
            ...
            try {
                ...
                $this->log('error migrating escape mediafile', backup::LOG_WARNING, 'course_files/'.$data['mediafile']);
            }
        }

        // start writing escape.xml
        $this->open_xml_writer("activities/escape_{$this->moduleid}/escape.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid, 'moduleid' => $this->moduleid,
            'modulename' => 'escape', 'contextid' => $contextid));
        $this->xmlwriter->begin_tag('escape', array('id' => $instanceid));
    ...
    public function on_escape_pages_start() {
        $this->xmlwriter->begin_tag('pages');
    }

    /**
     * This is executed every time we have one /MOODLE_BACKUP/COURSE/MODULES/MOD/ESCAPE/PAGES/PAGE
     * data available
     */
    public function process_escape_page($data) {
        global $CFG;
    ...
    /**
     * This is executed every time we have one /MOODLE_BACKUP/COURSE/MODULES/MOD/ESCAPE/PAGES/PAGE/ANSWERS/ANSWER
     * data available
     */
    public function process_escape_answer($data) {
        ...
        // upgrade step 2010121400 from mod/escape/db/upgrade.php
        $this->answers[] = $data;
    }

    public function on_escape_page_end() {
        ...
        if ( $firstbatch || $nextbatch ) { //we can write out 1 page atleast
            if ($this->prevpageid == 0) {
                // start writing with n-2 page (relative to this on_escape_page_end() call)
                $pg1 = $this->pages[1];
                $pg0 = $this->pages[0];
    ...
    public function on_escape_pages_end() {
        if ($this->pages) {
    ...
    /**
     * This is executed when we reach the closing </MOD> tag of our 'escape' path
     */
    public function on_escape_end() {
        // Append empty <overrides> subpath element.
        $this->write_xml('overrides', array());

        // finish writing escape.xml
        $this->xmlwriter->end_tag('escape');
        $this->xmlwriter->end_tag('activity');
        $this->close_xml_writer();

        // write inforef.xml
        $this->open_xml_writer("activities/escape_{$this->moduleid}/inforef.xml");
    ...
    protected function write_single_page_xml($page, $prevpageid=0, $nextpageid=0) {
        ...
        //effectively on_escape_answers_end(), where we write out answers for current page.
