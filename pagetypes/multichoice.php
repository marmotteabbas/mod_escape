...
define("ESCAPE_PAGE_MULTICHOICE",   "3");

class escape_page_type_multichoice extends escape_page {

    protected $type = escape_page::TYPE_QUESTION;
    protected $typeidstring = 'multichoice';
    protected $typeid = ESCAPE_PAGE_MULTICHOICE;
    ...
    public function display($renderer, $attempt) {
        ...
        if ($this->properties->qoption) {
            $mform = new escape_display_answer_form_multichoice_multianswer($action, $params);
        } else {
            $mform = new escape_display_answer_form_multichoice_singleanswer($action, $params);
        }
        $data = new stdClass;
        $data->id = $PAGE->cm->id;
        ...
        if ($this->properties->qoption) {
            ...
            foreach ($answers as $answer) {
                if ($this->escape->custom) {
                    ...
                    $iscorrectanswer = $this->escape->jumpto_is_correct($this->properties->id, $answer->jumpto);
                }
            ...
            if (!$answer = $DB->get_record("escape_answers", array("id" => $result->answerid))) {
    ...
    public function option_description_string() {
        if ($this->properties->qoption) {
            return " - ".get_string("multianswer", "escape");
        }
...
class escape_add_page_form_multichoice extends escape_add_page_form_base {
    ...
    public function custom_definition() {
        ...
        $this->_form->addElement('checkbox', 'qoption', get_string('options', 'escape'), get_string('multianswer', 'escape'));
        $this->_form->setDefault('qoption', 0);
        $this->_form->addHelpButton('qoption', 'multianswer', 'escape');
...
class escape_display_answer_form_multichoice_singleanswer extends moodleform {
    ...
    public function definition() {
        ...
        foreach ($answers as $answer) {
            ...
            if ($hasattempt && $answer->id == $USER->modattempts[$escapeid]->answerid) {
                $mform->setDefault('answerid', $USER->modattempts[$escapeid]->answerid);
            }
...
class escape_display_answer_form_multichoice_multianswer extends moodleform {
    ...
    public function definition() {
        ...
        if (isset($USER->modattempts[$escapeid]) && !empty($USER->modattempts[$escapeid])) {
            $hasattempt = true;
            $disabled = array('disabled' => 'disabled');
            $useranswers = explode(',', $USER->modattempts[$escapeid]->useranswer);
        }
