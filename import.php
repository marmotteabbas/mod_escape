...
require_once($CFG->dirroot.'/mod/escape/import_form.php');
require_once($CFG->dirroot.'/mod/escape/format.php');  // Parent class

$id     = required_param('id', PARAM_INT);         // Course Module ID
$pageid = optional_param('pageid', '', PARAM_INT); // Page ID

$PAGE->set_url('/mod/escape/import.php', array('id'=>$id, 'pageid'=>$pageid));
...
$strimportquestions = get_string("importquestions", "escape");
...
$mform = new escape_import_form(null, array('formats'=>escape_get_import_export_formats('import')));
$mform->set_data($data);
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->heading(format_string($escape->name), 2);
    echo $OUTPUT->heading_with_help($strimportquestions, 'importquestions', 'escape', '', '', 3);
...
if ($data = $mform->get_data()) {
    ...
    if (! $format->importpreprocess()) {
                print_error('preprocesserror', 'escape');
            }

    // Process the uploaded file
    if (! $format->importprocess($importfile, $escape, $pageid)) {
                print_error('processerror', 'escape');
            }

    // In case anything needs to be done after
    if (! $format->importpostprocess()) {
                print_error('postprocesserror', 'escape');
            }
