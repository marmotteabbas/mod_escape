...
/**
 * This page handles listing of escape overrides
 *
 * @package    mod_escape
...
require_once($CFG->dirroot.'/mod/escape/override_form.php');


$cmid = required_param('cmid', PARAM_INT);
$mode = optional_param('mode', '', PARAM_ALPHA); // One of 'user' or 'group', default is 'group'.

list($course, $cm) = get_course_and_cm_from_cmid($cmid, 'escape');
$escape = $DB->get_record('escape', array('id' => $cm->instance), '*', MUST_EXIST);
...
$url = new moodle_url('/mod/escape/overrides.php', array('cmid' => $cm->id, 'mode' => $mode));
...
require_capability('mod/escape:manageoverrides', $context);

// Display a list of overrides.
$PAGE->set_pagelayout('admin');
$PAGE->set_title(get_string('overrides', 'escape'));
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();
echo $OUTPUT->heading(format_string($escape->name, true, array('context' => $context)));

// Delete orphaned group overrides.
$sql = 'SELECT o.id
            FROM {escape_overrides} o LEFT JOIN {groups} g
            ON o.groupid = g.id
            WHERE o.groupid IS NOT NULL
              AND g.id IS NULL
              AND o.escapeid = ?';
$params = array($escape->id);
$orphaned = $DB->get_records_sql($sql, $params);
if (!empty($orphaned)) {
    $DB->delete_records_list('escape_overrides', 'id', array_keys($orphaned));
}
...
$overridedeleteurl = new moodle_url('/mod/escape/overridedelete.php');
$overrideediturl = new moodle_url('/mod/escape/overrideedit.php');
...
foreach ($overrides as $override) {
    ...
    if (isset($override->available)) {
        $fields[] = get_string('escapeopens', 'escape');
        $values[] = $override->available > 0 ?
                userdate($override->available) : get_string('noopen', 'escape');
    }

    // Format deadline.
    if (isset($override->deadline)) {
        $fields[] = get_string('escapecloses', 'escape');
        $values[] = $override->deadline > 0 ?
                userdate($override->deadline) : get_string('noclose', 'escape');
    }

    // Format timelimit.
    if (isset($override->timelimit)) {
        $fields[] = get_string('timelimit', 'escape');
        $values[] = $override->timelimit > 0 ?
                format_time($override->timelimit) : get_string('none', 'escape');
    }

    // Format option to try a question again.
    if (isset($override->review)) {
        $fields[] = get_string('displayreview', 'escape');
    ...
    if (isset($override->maxattempts)) {
        $fields[] = get_string('maximumnumberofattempts', 'escape');
    ...
    if (isset($override->retake)) {
        $fields[] = get_string('retakesallowed', 'escape');
    ...
    if (isset($override->password)) {
        $fields[] = get_string('usepassword', 'escape');
        $values[] = $override->password !== '' ?
                get_string('enabled', 'escape') : get_string('none', 'escape');
    }
...
echo html_writer::start_tag('div', array('id' => 'escapeoverrides'));
if (count($table->data)) {
    echo html_writer::table($table);
}
if ($hasinactive) {
    echo $OUTPUT->notification(get_string('inactiveoverridehelp', 'escape'), 'dimmed_text');
}
...
if ($groupmode) {
    if (empty($groups)) {
        // There are no groups.
        echo $OUTPUT->notification(get_string('groupsnone', 'escape'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'addgroup', 'cmid' => $cm->id)),
            get_string('addnewgroupoverride', 'escape'), 'post', $options);
} else {
    $users = array();
    // See if there are any users in the escape.
    ...
    if (empty($users)) {
        // There are no users.
        echo $OUTPUT->notification(get_string('usersnone', 'escape'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'adduser', 'cmid' => $cm->id)),
            get_string('addnewuseroverride', 'escape'), 'get', $options);
}
